---
name: health-advisor-clinical-decision-support
...
Clinical Decision Support is a skill for generating publication-ready, evidence-based clinical decision support documents for pharmaceutical and clinical research settings. It produces biomarker-stratified patient cohort analyses and GRADE-graded treatment recommendation reports with statistical rigor, designed for drug development, regulatory submissions, and clinical guideline development.

## Dependencies

- **scientific-schematics** — AI-generated figures (mandatory for every CDS document)
...
- **LaTeX/PDF toolchain** — Document compilation (tcolorbox, TikZ, pgfplots packages)
- **Python scripts** — `scripts/generate_survival_analysis.py` and others in `scripts/`
- **PubMed** — Medical literature references for evidence grading
...
Use the **scientific-schematics** skill directly to generate diagrams (the `scripts/generate_schematic.py` script is planned but not yet implemented).
...
See `../clinical-decision-support-archive/references/` for detailed guidance on:
...
See `assets/templates.tex` — a single consolidated file containing all LaTeX templates:
- **Section 1 — Color Schemes**: all color definitions (recommendation strength, urgency, biomarkers, CTCAE, Okabe-Ito accessible palette)
- **Section 2 — Biomarker Report**: molecular subtype classification and genomic profile reports
- **Section 3 — Clinical Pathway**: TikZ decision algorithm flowcharts for treatment sequencing
- **Section 4 — Cohort Analysis**: biomarker-stratified patient cohort analysis with statistical comparisons
- **Section 5 — Treatment Recommendation**: evidence-based clinical practice guidelines with GRADE grading
...
- **Applying cohort-level findings to individual patients** — This skill produces population-level analyses and evidence synthesis; do not use its outputs as individualized treatment plans. Use the `treatment-plans` skill for individual patients.
- **Omitting the executive summary first page** — Every CDS document must begin with a full-page executive summary with colored tcolorbox elements. Skipping this violates the mandatory document structure.
- **Reporting p-values without effect sizes** — Always include hazard ratios, confidence intervals, and clinical context alongside p-values. Isolated p-values are misleading.
- **Using unvalidated biomarker cut-points** — Biomarker stratification must specify assay methods, platforms, and validation status. Arbitrary cut-points undermine clinical credibility.
- **Generating documents without figures** — Every CDS document must include at least 1-2 AI-generated figures via the scientific-schematics skill. Text-only documents fail the mandatory visual requirement.
- **Including patient identifiers** — All 18 HIPAA identifiers must be removed before document generation. Never include real patient names, dates of birth, or medical record numbers.
...
- `create_waterfall_plot.py` - Best response visualization (planned, not yet implemented)
- `create_forest_plot.py` - Subgroup analysis visualization (planned, not yet implemented)
- `calculate_statistics.py` - Hazard ratios, Cox regression, log-rank tests (planned, not yet implemented)
- `grade_evidence.py` - Automated GRADE assessment helper (planned, not yet implemented)
