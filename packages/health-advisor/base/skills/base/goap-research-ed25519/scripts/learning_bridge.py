...
def _health_brain_is_distinct(project: Optional[str] = None) -> Tuple[bool, str]:
    ...
    # CONTAINMENT AGAINST THE PROTECTED STORE ITSELF (fix round 1, QE F1 — the FIFTH alias
    # shape). Pairwise INEQUALITIES are blind to NESTING: `ln -s .dz .health-brain` makes the
    # brain resolve one level INSIDE the shared store — `resolved_brain` (`<root>/.dz`) equals
    # neither `root` nor `shared_store`-as-compared-to-`resolved_store` (`<root>/.dz/.dz`), so
    # both equality checks above pass, containment below passes (the brain IS inside the
    # project), and the "already holds a dz store" probe keys on `<root>/.dz/.dz/memory`,
    # which does not exist until the FIRST leak creates it — a fail-open ordering. The one
    # question that closes the whole family: does ANY part of the brain resolve inside the
    # directory this property protects? Asked of the brain and of its store, so a deeper
    # `.health-brain/.dz -> .dz/<sub>` cannot slip between the two.
    for label, resolved in (("the brain", resolved_brain), ("the brain's store", resolved_store)):
        if os.path.commonpath([resolved, shared_store]) == shared_store:
            return False, (
                f"REFUSED — {HEALTH_BRAIN_DIRNAME} resolves INSIDE the shared store: {label} is "
                f"{resolved}, under {shared_store}.\n"
                "  Health lessons must not land in the shared store, and a symlink here would "
                "put them there — on the FIRST write, before any probe could key on the "
                "evidence a leak creates. Remove or rename it, then teach again."
            )
...
def _brain_count(brain: str, timeout: int = 90) -> Optional[int]:
    """How many lessons the health brain holds, or None if it cannot be read.

    `timeout` is EXPLICIT at the `ingest-documents` call sites (ADR-001 D-8a / SP-22): that verb runs
    under a 120 s outer kill in `write.js`, and this function's own 90 s default plus the teach call's
    60 s would sum past it — so the outer kill would pre-empt a precise inner refusal with the vaguer
    "bridge timeout". `teach()` keeps the default; it has no outer bound to stay under.

    HEALTH_BRAIN_COUNT_STUB is a TEST-RESERVED SEAM, sanctioned in ADR-001 § D-3 (2026-08-18, P2-d),
    and it is read at exactly this one site — never a parallel implementation of the count:
        unset      -> the real count (production, always)
        "none"     -> None, i.e. an UNREADABLE brain, so the fail-closed branch can be PROVEN
        an integer -> that value, so "the count did not grow by `written`" can be PROVEN
    The alternative to the seam is a fail-closed branch nobody can exercise, which is a claim rather
    than a check. The scope is pinned here rather than left to taste.
    ...
    """
    stub = os.environ.get("HEALTH_BRAIN_COUNT_STUB")
    if stub is not None:
        if stub.strip().lower() == "none":
            return None
        try:
            return int(stub.strip())
        except ValueError:
            return None
    code, out, _ = _run_dz(["recall", "--all", "--stats", "--project", brain,
                            "--include-domain", LEARNING_DOMAIN], timeout=timeout)
    if code != 0:
        return None
...
# ------------------------------------------------- ingest-documents (ha third-brain, ADR-001 D-3)

# The two lines `dz teach --from-json` emits, and the ONLY two this parser reads. MEASURED —
# reproducer: `sed -n '2398,2399p' packages/@dzhechkov/harness-cli/src/cli.ts`:
#     Imported <k> pattern(s) from <path>
#       Skipped <j> (duplicates already in the store, or invalid entries)
# They are the PRIMARY source for `written`/`skipped` (ADR-001 D-3, P1-3); the before/after count
# delta is the cross-check BELT over them, never their source. A count delta cannot tell "3 written"
# from "5 written and 2 forgotten by something else", and the dedup layer's own `skipped` number is
# not derivable from a count at all.
_IMPORTED_RE = re.compile(r"^Imported (\d+) pattern\(s\)", re.MULTILINE)
_SKIPPED_RE = re.compile(r"^\s*Skipped (\d+) \(", re.MULTILINE)

# The one line of foreign stdout the parser must NOT interpret: `ℹ … near-duplicate cluster(s)` from
# the post-import `harmonizeVectorStore(projectRoot, {})` call, which runs OUTSIDE `--no-mirror`'s
# guard. It is a DRY RUN — `const apply = opts.apply === true` (MEASURED — reproducer
# `sed -n '1182p' packages/@dzhechkov/harness-core/src/vector-tier.ts`) — so it previews and writes
# nothing. Skipped by construction: neither regex above can match it.

# Inner timeouts, EXPLICIT rather than inherited (ADR-001 D-8a, AM-7/SP-22). The module defaults are
# 60 s (`_run_dz`) and 90 s (`_brain_count`), summing to 240 s across this verb's three dz calls —
# past the 120 s outer `BRIDGE_TIMEOUT_MS` in write.js, which would let the outer kill pre-empt a
# precise inner refusal. 30 + 45 = 75 s is strictly under it, with room for the un-timed local gates.
INGEST_TEACH_TIMEOUT = 30
INGEST_COUNT_TIMEOUT = 45


def ingest_documents(payload_path: str, project: Optional[str] = None) -> Tuple[int, str]:
    """File a batch of FULL analytical-document passages into the segregated health brain.

    ONE GATE, EXTENDED — NEVER FORKED. This verb calls the same four fence functions `teach()` calls,
    unchanged and in the same order, and then calls `dz teach --from-json` as a CLI PRIMITIVE. There
    is no code path from here through `teach()` at all.

    WHY `check_lesson()` IS NOT CALLED HERE, AND MUST NOT BE ADDED (ADR-001 D-3 / AM-1 / INV-11).
    `check_lesson` refuses IDENTIFIER FORMATS — an email, a phone number, a medical-record number, a
    card number — because a distilled METHOD LESSON has no business containing one. A full analytical
    document is the opposite case: a lab report legitimately carries dated analyte values, an
    accession number and a record identifier, and refusing it would refuse the feature. The guarantee
    for documents is the STORE SEPARATION (the four checks below), exactly as this file's own header
    argues for lessons: "a store that never receives the data cannot leak it". Do NOT "fix" this back
    by adding the format check — that would not add a defence, it would delete the feature.

    `project` IS REQUIRED by the CLI parser (never defaulted to cwd, unlike `teach`/`recall`): a
    defaulted project on a BATCH write is precisely the shared-store-by-accident class D1 exists to
    prevent, and there is no default here to get wrong.

    Returns (exit_code, json_verdict). The verdict is the JS side's ONLY input — parsed, never
    screen-scraped.
    """
    def refuse(reason: str, message: str, **extra) -> Tuple[int, str]:
    ...
    if not os.path.isfile(payload_path):
        return refuse(
            "third_brain_payload_escape",
            f"REFUSED — the payload {payload_path} is not a readable regular file. Nothing was written.",
        )

    # 1 — distinctness, on REALPATHS, checked at write time because a symlink can appear after the
    #     process starts. This is the invariant the whole design rests on.
    distinct, brain_or_why = _health_brain_is_distinct(project)
    if not distinct:
        return refuse("third_brain_not_segregated", brain_or_why)
    brain = brain_or_why

    # 2 — the brain exists and is self-ignoring for git.
    try:
        _protect_brain(brain)
    except (RuntimeError, OSError) as exc:
        return refuse("third_brain_not_segregated", f"REFUSED — {exc}")

    if dz_path() is None:
        # A DELIBERATE DIVERGENCE from teach()'s NOTE posture (ADR-001 D-5): an absent `dz` is an
        # acceptable no-op for an optional lesson loop and a hard failure here, because an operator
        # who ran `ha third-brain ingest` believes the document was filed.
        return refuse("third_brain_dz_unavailable", f"REFUSED — {DZ_MISSING_NOTE.strip()}")

    # 3 — PRE-FLIGHT: prove the write goes where we asked, using a non-medical canary, BEFORE any
    #     patient text is anywhere near `dz`.
    routes, why = _project_write_lands(brain, project)
    if not routes:
        return refuse("third_brain_shared_store_targeted", f"REFUSED — {why}")

    # 4 — the belt: the brain's own count, before and after.
    before = _brain_count(brain, timeout=INGEST_COUNT_TIMEOUT)
    code, out, err = _run_dz(
        ["teach", "--from-json", payload_path, "--project", brain, "--no-mirror"],
        timeout=INGEST_TEACH_TIMEOUT,
    )
    if code == 127:
        return refuse("third_brain_dz_unavailable", f"REFUSED — {err.strip() or DZ_MISSING_NOTE.strip()}")
    if code != 0:
        return refuse(
            "third_brain_write_unverified",
            f"REFUSED — `dz teach --from-json` exited {code}: {err.strip() or out.strip() or '(no output)'}",
        )

    m_written = _IMPORTED_RE.search(out)
    m_skipped = _SKIPPED_RE.search(out)
    if m_written is None or m_skipped is None:
        # PARSE, NEVER SYNTHESISE. Output this side cannot read is not evidence of a successful
        # write; treating it as one is exactly how a silent failure reads as a clean run.
        return refuse(
            "third_brain_write_unverified",
            "REFUSED — `dz teach --from-json` exited 0 but did not report what it did: the "
            "`Imported <k> pattern(s)` / `Skipped <j> (…)` lines are absent or unparseable, so this "
            f"bridge cannot say whether the batch landed. Raw output:\n{out.strip() or '(empty)'}",
        )
    written = int(m_written.group(1))
    skipped = int(m_skipped.group(1))

    after = _brain_count(brain, timeout=INGEST_COUNT_TIMEOUT)
    # FAIL CLOSED when the count is unavailable — teach()'s own posture, and for its own reason: an
    # unreadable brain is not evidence the write landed.
    if before is None or after is None:
        return refuse(
            "third_brain_write_unverified",
            "REFUSED — the write could not be VERIFIED: the health brain's count is unreadable "
            f"({brain}). `dz` reported success, but this bridge cannot confirm the batch landed "
            "there rather than in the shared store, so it does not claim it did.",
            written=written, skipped=skipped,
        )
    # A re-ingest the dedup layer skips ENTIRELY (`written: 0`, count unchanged) is a SUCCESS
    # (ADR-001 D-3 §4 / SP-11). Idempotency is the requirement; a fail-closed check that ate it would
    # be a check mis-aimed at its own feature. The refusal fires only when the bridge's own report
    # says something WAS written and the count does not corroborate it.
    if written > 0 and (after - before) != written:
        return refuse(
            "third_brain_write_unverified",
            f"REFUSED — `dz` reported {written} record(s) imported but the health brain went from "
            f"{before} to {after} ({after - before:+d}). The batch may have gone to the SHARED store "
            f"instead of {brain}. Check the shared store, then upgrade: npm i -g @dzhechkov/harness-cli",
            written=written, skipped=skipped, before=before, after=after,
        )
    return 0, json.dumps({
        "ok": True, "brain": brain, "before": before, "after": after,
        "written": written, "skipped": skipped,
    })
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # ha third-brain's write leg (ADR-001 D-3). `--project` is REQUIRED and has NO default: unlike
    # `teach`/`recall`, which fall back to the cwd, a defaulted project on a batch write is exactly
    # the shared-store-by-accident class the segregation invariant exists to prevent.
    p_ingest = sub.add_parser("ingest-documents")
    p_ingest.add_argument("--payload", required=True,
                          help="a JSON array of {pattern,type,reward,domain,ts} records")
    p_ingest.add_argument("--project", required=True,
                          help="the workspace whose .health-brain receives the batch (never defaulted)")
    p_ingest.add_argument("--json", action="store_true", dest="json_out",
                          help="print the machine verdict (the JS side parses it; it is printed either way)")
    ...
    if args.cmd == "recall":
        ...
        return 0
    if args.cmd == "ingest-documents":
        code, verdict_json = ingest_documents(args.payload, args.project)
        print(verdict_json)
        return code
    if args.cmd == "check":
