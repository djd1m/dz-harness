...
`third-brain` и `intake-archive` поставляются только как документация: их движкам нужны зависимости за пределами каталога навыка, которые не копируются при отдельной установке.
...
## Patient values in Solve: предпочтения без инверсии безопасности

Health Advisor принимает значения пациента в версионированном блоке `patient-values-v1`:
...
```json
{
  ...
  "as_of": "2026-08-30",
  "preferences": [
    {"id":"pv-no-statins","kind":"avoid","dimension":"drug_class","value":"statin","priority":1,"statement":"не хочу статины","reason":null},
    {"id":"pv-non-pharm","kind":"prefer","dimension":"treatment_approach","value":"non_pharmacological","priority":2,"statement":"предпочитаю немедикаментозное","reason":null},
    {"id":"pv-cost","kind":"constraint","dimension":"cost","value":"critical","priority":3,"statement":"стоимость критична","reason":null}
  ],
  "life_context": {"pregnancy_intent":"planning"}
}
```

`patient_values.as_of` — дата подтверждения пациентом. Когда `solve-envelope-v1.as_of` задан, значения
старше 30 дней, с отсутствующей/невалидной или будущей датой требуют повторного подтверждения и не
влияют на ранжирование. Если именно Solve-оболочка не содержит `as_of`, датированные значения остаются
применёнными и видимыми, а состояние документа и сами значения помечаются `freshness_unknown` с явным предупреждением
`solve_envelope.as_of`; они никогда не объявляются текущими. Намерение беременности — отдельный
клинический контекст, а не «балл предпочтения». Отсутствующий блок сохраняет состояние
`absent`; подтверждённый блок без предпочтений — `empty`; оба оставляют исходный порядок неизменным.
В частично невалидном блоке валидные соседние поля продолжают работать, а каждое исключённое поле
печатается как `PATIENT VALUES INPUT WARNING` и имеет нулевой эффект.

### Safety before values

Сначала клиническая линия фиксирует экстренные действия, красные флаги, противопоказания,
показания и полное множество клинически приемлемых вариантов. Только затем значения пациента могут
стабильно переставить **все и только** приемлемые варианты. Они не являются фильтром: не могут
скрыть красный флаг, сделать противопоказанный вариант приемлемым, убрать показанный класс или
отменить клиническую цену отказа. При экстренном действии ранжировщик предпочтений не вызывается.

Например, «не хочу статины» не удаляет показанный вариант. Любое фактическое понижение показанного
варианта из-за `avoid`, `prefer` или `constraint` печатает причинившее его предпочтение, само показание,
ссылку на доказательство и последствие отказа; альтернативы могут подняться в списке, но обе истины
остаются видимы. Экстренный merge дополнительно требует точного исходного порядка даже при пустом
`option_effects`, а `clinical_integrity` считается по исходной клинической линии до merge и сверяется
с результатом до глубокой заморозки.

### How Patient Values Changed the Ranking

Результат выводит каждое учтённое значение и точные позиции до/после, например:

```text
- Statin therapy: position 1 → 2; pv-non-pharm: neutral (...).
- Structured lifestyle programme: position 2 → 1; pv-non-pharm: supports (...).
```

Если перестановки не было, печатается `No option changed rank.` — отдельного флага «успешно
персонализировано» нет. Невалидные и непереоценённые значения также видны в том же исходном
Markdown, поэтому объяснение нельзя получить только из README или необязательного HTML.

Варианты `requires_clinician` и `contraindicated`, содержащие показание, его обоснование или клиническое
последствие отказа, всегда выводятся в собственных разделах и не зависят от наличия дублирующей строки
в массивах `indications`/`contraindications`. Заголовки локализованы: `locale: "ru"` печатает все
заголовки раскрытия ценностей по-русски, `locale: "en"` — по-английски; отсутствующая или неизвестная
локаль использует английские заголовки по умолчанию.

Канонический zero-dependency merge находится в `base/solve/solve-advice.js` в дереве пакета и в
`solve/solve-advice.js` внутри свежей base-установки мастер-скилла:

```bash
node solve/solve-advice.js < solve-envelope.json > solve-advice.md
```

На вход подаётся `solve-envelope-v1`; при структурной ошибке команда отказывает без частичного
Markdown. Каждый успешный путь безусловно печатает границу: patient values **do not replace a doctor**
и не отменяют показания, противопоказания, красные флаги или совет обратиться за срочной помощью.
Это информационная поддержка разговора с квалифицированным врачом, а не медицинское назначение.
