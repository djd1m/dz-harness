 ...
 */
/** Everything this function touches outside itself. A test supplies all four. */
export interface PublishedFetchDeps {
    /** Create the temp directory. Production: `mkdtempSync(join(tmpdir(), prefix))`. */
    readonly makeTempDir: () => string;
    /** Run a shell command; throwing means failure, exactly as `execSync` does. */
    readonly run: (command: string) => void;
    /** List a directory's entries. */
    readonly listDir: (dir: string) => readonly string[];
    /** Register a temp directory for cleanup. Called BEFORE anything that can throw. */
    readonly onTempDir: (dir: string) => void;
    /** Join path segments. Production: `node:path`'s `join`. */
 ...
 */
export declare function fetchPublishedViaNpmPack(name: string, version: string, deps: PublishedFetchDeps): {
    dir: string;
} | null;
//# sourceMappingURL=sibling-drift-fetch.d.ts.map