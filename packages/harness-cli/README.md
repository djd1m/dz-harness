...
**Experiment envelope (`--envelope`, ADR-001 envelope-before-dispatch).** The feature-adr conveyor
builds a small JSON object once per run — task kind, tier, priority, which model arms it considered,
which one it chose, and who is judging it (`harness-core`'s `feature-adr-envelope.ts`) — and hands it
to `round open --envelope '<json>'` so the round's own ledger row carries the same decision the
autowritten stage rows carry. `close` copies it into the `stage:"round"` ledger row unchanged; a round
opened without `--envelope` closes with no `envelope` field at all (unaffected, exactly as before this
flag existed). Example:
```
dz round open --slug add-x --round 1 --topic "add X" \
  --envelope '{"schema":1,"runId":"fa:add-x:20260916T000000Z","attempt":1,"taskKind":"feature","tier":"M","priority":"balance","treeSha":null,"treeShaReason":"no git in sandbox","arms":{"mode":["same-family","cross-family"],"stages":{"code":["sonnet","opus"]}},"chosen":{"mode":"cross-family","stages":{"code":"sonnet"}},"policy":{"name":"routing-tables","version":"routing-tables/2026-09-16","propensity":null},"evaluator":{"family":"codex","model":"gpt-5.6-sol","source":"planned"}}'
# ... work happens ...
dz round close --slug add-x --round 1 --outcome shipped --grade B+ --lesson teach:abc123
# the ledger row for this round now carries "envelope": { ...the same object... } and "grade":"B+"
```
An invalid `--envelope` (malformed JSON, or JSON that fails `validateExperimentEnvelope`) refuses
`open` with exit 2 and names the first invalid field — no state is written.

**Grade is mandatory for a finished review (`--grade`, measurement-integrity FR-7/ADR-001 D5).**
`close --outcome shipped|refuted` now REQUIRES `--grade <A|A-|B+|…>` — omit it and `close` refuses,
exit 2, `grade required for a finished review`, and NOTHING is written (the old always-`null` grade
column is gone). `close --outcome blocked|abandoned` treats a passed `--grade` as a no-op: the close
still succeeds, but the grade is DROPPED (a round that never finished has nothing to grade) and a
warning line (`⚠ --grade "…" проигнорирован: …`) names why. When `--reviewer` is absent, `close`
looks for the LATEST (by `emittedAt`) `features/<slug>/.fa-state/qe-bridge/signoff-*.json` for this
slug and fills `reviewer` (family:model), `reviewMinutes` (`elapsedMs / 60000`) and
`reviewSource:'qe-bridge'` from it — an explicit `--reviewer` always wins and carries neither derived
field. A `--grade` that disagrees with that sidecar's OWN grade is refused, naming both:
```
dz round close --slug add-x --round 1 --outcome shipped
# grade required for a finished review: outcome=shipped требует --grade <A|A-|B+|…>  (exit 2, nothing written)
...
dz score             --slug <feature> [--project <dir>] [--json]   # process scorecard for ONE feature-adr run, from its artifacts: ADR confirmation, discrimination proof, cross-model QE grade, live verification, README-first, learning loop, amendments — DESCRIPTIVE-ONLY (a low score exits 0); evidence lines are shown so the reader judges the heuristics. qe-findings-record FR-4: `qeGrade`/`gradeSource` prefer a machine-written `QE-VERDICT: <grade>` line over the prose scan ('verdict-line' beats 'prose'; two verdict lines are 'ambiguous', never "last wins"), and `findings` reports the report's `## Findings ledger` table — `{status:'absent'}` when there is none (406 pre-existing reports), else `{hollow, summary:{bySeverity,byStatus,total}, refused}` with a REFUSED (never coerced) entry for every row outside the closed Severity/Status/Author dictionaries; text mode prints a `findings: N HIGH / M MEDIUM; K refused (line L: reason)` line
dz recap             [--day|--week|--month] [--at <ISO date>] [--refresh-publishes] [--project <dir>] [--json]   # what was done over a window, from records only. `--refresh-publishes` fills the third-party publish-times cache first (the ONLY place this command touches the network — 51 packages in ~7s, batches of 8); the report itself always reads the cache, prints its AGE, and names any package the registry did not answer for: those are MISSING from the numbers, not zero. Deliveries carry the grade an independent review STATED — a report naming two different grades is reported AMBIGUOUS, never guessed (qe-findings-record FR-4: a machine-written `QE-VERDICT: <grade>` line wins over the prose scan when present; two verdict lines are still AMBIGUOUS, never "last wins"); registry publishes come from a cache (51 packages cost 18.3s over the network, MEASURED — never inside a report); gate verdicts and knowledge reuse come from the local stores. --quarter/--half-year/--year are RECOGNISED and REFUSED with the real span in days: there is exactly ONE complete quarter and the longest record is 174 days, so a quarter-over-quarter comparison is arithmetically impossible and a year would be fabrication. Every section carries its own data-start date, and "the source was not read" never prints as a zero. Contaminated measures are NOT computed and the report says so: commit count (this project mandates a commit per logical change), lines changed (352 of 1318 commits are docs), token spend (self-declared an estimate, once wrong sixfold), learning-event volume (the curve tracks when hooks were installed), inventory counts (monotonic — they can only flatter), lesson count (54% of the pool has never been read). exit 0 reported / 2 refused
...
dz publish: tarball @dzhechkov/harness-cli@0.8.30 sha256:d656…c334
...
readable verdict line and findings ledger that `dz score`/`dz recap` read; `dz round close --grade` is mandatory for
shipped/refuted; automatic ledger rows carry the experiment envelope; the feature-adr workflow parses again as an ES
