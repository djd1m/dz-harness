...
import {
  ...
  loadSubsystemManifest,
  loadProductVision,
  checkFeatureAgainstArchitecture,
  renderArchCheck,
  planProjectSkills,
  guidanceForStage,
 ...
 */
function discoverSkillsDirs(cwd: string, explicitSkillsDir?: string | undefined): string[] {
  if (explicitSkillsDir !== undefined) return [resolve(cwd, explicitSkillsDir)];
...
async function installSkills(opts: {
  ...
  projectRoot: string;
  cwd: string;
  explicitSkillsDir?: string | undefined;
...
async function cmdScout(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  ...
  if (showReport) {
    ...
    if (!saved) { write('dz scout: no saved report. Run `dz scout` first.'); return 1; }
  ...
  try {
    ...
    let deepReport: Awaited<ReturnType<typeof deepAnalyze>> | undefined;
...
function cmdBundle(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  ...
  for (const b of r.bundled) {
    for (const w of b.warnings) write(`  ⚠ ${w}`);
  }
...
function cmdCompose(options: Map<string, string>, cwd: string, write: Write, writeErr: WriteErr): number {
  ...
  for (const name of presetNames) {
    ...
    for (const skill of preset.skills) allSkills.add(skill);
...
function cmdDiff(options: Map<string, string>, cwd: string, write: Write): number {
  ...
  if (existsSync(baseDir)) {
    for (const pack of readdirSync(baseDir, { withFileTypes: true })) {
      if (!pack.isDirectory() || !pack.name.startsWith('skills-')) continue;
      const candidate = join(baseDir, pack.name, skillId, 'SKILL.md');
      if (existsSync(candidate)) { canonicalPath = candidate; break; }
    }
  }
  ...
  for (let i = 0; i < Math.max(instLines.length, canLines.length) && diffs < 5; i++) {
    if (instLines[i] !== canLines[i]) {
      write(`  Line ${i + 1}:`);
      if (instLines[i] !== undefined) write(`    - ${instLines[i]?.slice(0, 80)}`);
      if (canLines[i] !== undefined) write(`    + ${canLines[i]?.slice(0, 80)}`);
      diffs++;
    }
  }
  if (diffs >= 5) write(`  ... (showing first 5 differences)`);
 ...
 */
function statuslineGitBranch(projectRoot: string): string | undefined {
  try {
 ...
 */
function cmdStatuslineInstall(options: Map<string, string>, cwd: string, write: Write): number {
  const projectRoot = resolve(cwd, options.get('project') ?? '.');
...
function cmdStatuslineFaRecord(options: Map<string, string>, cwd: string, write: Write): number {
  ...
  const parseCount = (name: string): number | undefined => {
...
async function cmdConsolidate(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  ...
  const sourceLabel: Record<typeof result.source, string> = {
  ...
  if (result.harvested === 0) {
    ...
    if (result.watermark !== undefined) write(`  Watermark: ${result.watermark}`);
    // Option C: vector-index mirror status (honest — shows why when skipped/failed).
    if (result.mirrored > 0) write(`  Mirrored to agentdb vector index: ${result.mirrored}`);
    if (result.mirrorError !== undefined) write(`  Mirror: ${result.mirrorError}`);
  }
  ...
  try {
    ...
    if (delta.scored > 0) {
      ...
      for (const r of delta.rising) write(`    ↑ ${r.delta >= 0 ? '+' : ''}${r.delta.toFixed(3)}  ${r.pattern.replace(/\s+/g, ' ').slice(0, 64)}`);
      if (delta.stale.length > 0) {
        write(`    stale (slope ≤ 0 — prune CANDIDATES, not deleted; review with dz recall):`);
        for (const r of delta.stale) write(`    ↓ ${r.delta.toFixed(3)}  ${r.pattern.replace(/\s+/g, ' ').slice(0, 64)}`);
      }
    }
...
function renderHarmonize(report: HarmonizeReport, write: Write): void {
...
async function cmdVector(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  ...
  if (sub === 'reindex') {
    ...
    if (report.backupPath !== undefined) write(`  snapshot: ${report.backupPath}`);
 ...
 */
function resolveDepsRoot(cwd: string): string {
  const canResolve = (root: string): boolean => {
    try {
...
async function cmdBrain(
  ...
  if (sub === 'list') {
    ...
    if (asJson) { write(JSON.stringify(sources)); return 0; }
    if (sources.length === 0) {
  ...
  if (sub === 'update') {
    ...
    if (asJson) { write(JSON.stringify(result)); return result.error !== undefined ? 1 : 0; }
...
async function cmdAutoCanonicalize(options: Map<string, string>, cwd: string, write: Write): Promise<number> {
  const source = options.get('source');
  ...
  try {
    ...
    const headers: Record<string, string> = { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'dz-harness' };
    ...
    const tree = (await resp.json()) as { tree: { path: string; type: string }[] };
...
function cmdParity(options: Map<string, string>, flags: Set<string>, write: Write, writeErr: WriteErr): number {
  ...
  const short: Record<string, string> = TARGET_SHORT_LABELS;
...
function cmdBenchmark(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
 ...
 */
function walkMarkdownFiles(dir: string, depth = 0): string[] {
  const out: string[] = [];
  ...
  try {
    ...
    for (const e of entries) {
      ...
      if (e.isSymbolicLink()) continue; // never follow links — cycle-proof
      const full = join(dir, e.name);
      if (e.isDirectory()) {
        for (const f of walkMarkdownFiles(full, depth + 1)) out.push(f);
...
function defaultClaimScanSet(root: string): string[] {
  ...
  const add = (p: string): void => {
    if (!seen.has(p)) { seen.add(p); set.push(p); }
  };
  const rootReadme = join(root, 'README.md');
  if (existsSync(rootReadme)) add(rootReadme);
  try {
    for (const p of discoverPackages(root)) {
      const readme = join(p.dir, 'README.md');
      if (existsSync(readme)) add(readme);
    }
  ...
  try {
    ...
    if (existsSync(featuresDir)) {
      for (const e of readdirSync(featuresDir, { withFileTypes: true })) {
        ...
        if (existsSync(qe)) add(qe);
      }
    }
  } catch { /* no features dir — skip */ }
  for (const f of walkMarkdownFiles(join(root, 'docs'))) add(f); // docs/ — live, published guidance
  return set;
}
...
function cmdClaimCheck(
  ...
  if (findings.length > 0) {
    const groups = new Map<string, { findings: number; high: number }>();
    for (const f of findings) {
      const key = claimGroupKey(f.file);
      const g = groups.get(key) ?? { findings: 0, high: 0 };
      g.findings += 1;
      if (f.severity === 'high') g.high += 1;
      groups.set(key, g);
    }
    const rows = [...groups.entries()].sort((a, b) => b[1].findings - a[1].findings || (a[0] < b[0] ? -1 : 1));
    write(`\n  Findings by group:`);
    for (const [key, g] of rows) write(`    ${g.findings} finding(s), ${g.high} high — ${key}`);
  }
  return computeClaimExit(findings, failOn);
}
 ...
 */
function claimGroupKey(file: string): string {
  if (file === 'README.md') return 'README.md';
  const parts = file.split('/');
  if (parts[0] === 'features') return parts.length > 1 ? `features/${parts[1]}` : 'features';
  if (parts[0] === 'docs') return parts.length > 2 ? `docs/${parts[1]}` : 'docs';
  if (parts[0] === 'packages') return parts.slice(0, -1).join('/');
  return file;
}
...
function cmdMcpScan(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  ...
  for (const key of ['json', 'reconcile', 'fail-on-undergrant'] as const) {
    const v = options.get(key);
    if (v === undefined || v === 'true') continue;
    if (key === 'json') json = true;
    else if (key === 'reconcile') reconcile = true;
    else failUnder = true;
    if (pathArg === undefined && existsSync(resolve(cwd, v))) pathArg = v;
  }
  ...
  if (json) {
    ...
    if (rec) emitPolicyIfRequested(rec, root, options, flags, write); // runs in CI/json mode too
    return exit();
  }
  ...
  write(
    `   shell=${cap.shell} network=${cap.network} file-write=${cap.fileWrite} secrets-reachable=${cap.secretsReachable} default-deny=${cap.defaultDeny}`,
  );
...
function renderReconcile(rec: ReturnType<typeof reconcileCapabilities>, write: Write): void {
  ...
  if (rec.findings.length === 0) {
    ...
    for (const f of rec.findings) {
      ...
      if (f.skills.length > 0) write(`           skills: ${f.skills.join(', ')}`);
    }
  }
  if (rec.limits) {
    const l = rec.limits;
 ...
 */
function emitPolicyIfRequested(
  rec: ReturnType<typeof reconcileCapabilities>,
  root: string,
  options: Map<string, string>,
  flags: Set<string>,
  write: Write,
): void {
  if (!flags.has('emit-policy') && options.get('emit-policy') === undefined) return;
...
async function cmdSyncUpstream(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
 ...
 */
function readDriftAllowlist(root: string): string[] {
  const p = join(root, '.dz', 'drift-allowlist.json');
  if (!existsSync(p)) return [];
  try {
    const raw = JSON.parse(readFileSync(p, 'utf8'));
    const arr = Array.isArray(raw) ? raw : Array.isArray(raw?.skills) ? raw.skills : [];
    return arr.map((e: unknown) => (typeof e === 'string' ? e : (e as { name?: string })?.name)).filter((n: unknown): n is string => typeof n === 'string' && n.length > 0);
  } catch {
    return [];
  }
}
...
function gatherGuardFacts(op: string, root: string, text: string | undefined, storeCap: number): Record<string, unknown> {
  ...
  if (op === 'publish') {
    ...
    try {
      ...
      if (!existsSync(showcaseDir)) {
        ...
        if (hasPlugin !== hasMarketplace) {
          ...
          if (hasPlugin) {
            try {
              const partial = JSON.parse(readFileSync(pluginPath, 'utf8')) as { version?: unknown };
              if (typeof partial.version === 'string' && partial.version !== '') publishedVersion = partial.version;
            } catch { /* the half-showcase violation remains valid without a parseable version */ }
          }
          ...
          const publishedPlugin = JSON.parse(readFileSync(pluginPath, 'utf8')) as { version?: unknown };
          const publishedMarketplace = JSON.parse(readFileSync(marketplacePath, 'utf8')) as unknown;
          const publishedVersion = typeof publishedPlugin.version === 'string' && publishedPlugin.version !== ''
            ? publishedPlugin.version
            : undefined;
    ...
    } catch { /* fail-open: malformed/unreadable showcase evidence is not a drift verdict */ }
 ...
 */
function cmdSyncCanonical(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  const root = resolve(cwd, options.get('project') ?? '.');
  ...
  if (flags.has('json')) {
    write(
      JSON.stringify({ ...r, canonical: relative(root, r.canonical), wrote: r.wrote.map((w) => relative(root, w)) }),
    );
    return exitCode;
  }
  ...
  if (r.resolvedFrom === 'auto') {
    ...
    for (const w of r.wrote) write(`⚠     ${relative(root, w)}`);
  ...
  for (const w of r.wrote) write(`  SYNCED ${relative(root, w)}`);
  write(`synced ${r.synced}; ${r.copies - r.drifted} already matched.`);
  return 0; // default: overwrite + prove identity, exit 0
}

function cmdPlugin(options: Map<string, string>, cwd: string, write: Write): number {
  const registry = buildRegistry(cwd);
...
function cmdArchitecture(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  ...
  if (flags.has('check')) {
    ...
    const result = checkFeatureAgainstArchitecture(
      { slug, description, proposedCommands, ...(subsystem ? { targetSubsystem: subsystem } : {}) },
      map,
      loadProductVision(repoRoot),
    );
...
function gitTrackedPackages(repoRoot: string): Set<string> {
 ...
 */
async function cmdMrRakes(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  let repoRoot = cwd;
  try {
  ...
  if (flags.has('json')) { write(JSON.stringify(report, null, 2)); return 0; }
  ...
  if (flags.has('teach')) {
    if (confirmedRakes.length === 0) { write('\n↳ --teach: no confirmed rakes to teach.'); return 0; }
...
async function cmdRetro(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  ...
  if (!transcript) { write('dz retro: no session transcript found — pass a path, or run from a project with roam state.'); return 1; }
  ...
  if (flags.has('json')) { write(JSON.stringify(retro, null, 2)); return 0; }
 ...
 */
function cmdLedgerBackfill(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  const json = flags.has('json');
  ...
  // One host-record lookup per runId, cached: deriveCostLedger walks the transcript tree.
  type Facts = { tokens: number | null; minutes: number | null; agents: number | null };
  const cache = new Map<string, Facts | typeof AMBIGUOUS | null>();
  ...
  const factsFor = (runId: string): Facts | null => {
    try {
  ...
  const facts = (key: 'runId' | 'slug', value: string): Facts | typeof AMBIGUOUS | null => {
    const ck = key + ':' + value;
    const hit = cache.get(ck);
    if (hit !== undefined) return hit;
    let out: Facts | typeof AMBIGUOUS | null = null;
    if (key === 'runId') {
      out = factsFor(value);
    } else {
      ...
      try {
        ...
        if (matches.length > 1) out = AMBIGUOUS;
        else if (matches.length === 1 && matches[0] !== undefined) out = factsFor(matches[0].runId);
      } catch { out = null; }
    }
    cache.set(ck, out);
    return out;
  ...
  for (const r of unfilled) byReason.set(String(r.skipped), (byReason.get(String(r.skipped)) ?? 0) + 1);
  for (const [reason, n] of byReason) write(`  not filled: ${n} row(s) — ${reason}`);
  return 0;
}
...
function cmdFeatureAdrRecord(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): number {
  ...
  if (flags.has('backfill')) return cmdLedgerBackfill(options, flags, cwd, write);
...
function cmdStats(cwd: string, write: Write): number {
  ...
  if (existsSync(join(cwd, '.dz'))) {
    ...
    try {
      consolidated = (JSON.parse(readFileSync(join(cwd, '.dz', 'memory', 'consolidate.json'), 'utf-8')) as { lastConsolidatedTs?: string }).lastConsolidatedTs;
...
function cmdDashboard(cwd: string, write: Write): number {
  const baseDir = join(cwd, 'packages', '@dzhechkov');
  if (!existsSync(baseDir)) {
  ...
  const skillPacks: { name: string; count: number }[] = [];
  const core: string[] = [];
...
async function cmdImportEcc(options: Map<string, string>, flags: Set<string>, cwd: string, write: Write): Promise<number> {
  ...
  const importOpts: Parameters<typeof importEcc>[0] = {
    existingSkillsDir,
    outputDir,
    force: flags.has('force'),
  ...
  if (report.imported > 0) {
    ...
    if (report.imported > 20) write(`║    ... and ${report.imported - 20} more${' '.repeat(45)}║`);
  }
