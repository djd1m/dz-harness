...
function showKeysariumIntegration(keysariumManifest) {
  ...
  console.log(cyan('  \u2502') + ' Idea2PRD Manual integrates with existing Keysarium.  ' + cyan('\u2502'));
...
async function run(options) {
  ...
  if (fileExists(manifestPath)) {
    if (!force) {
      warn('Idea2PRD Manual skill pack is already installed in this directory.');
      info(`Run ${cyan('@dzhechkov/skills-idea2prd update')} to update, or use ${yellow('--force')} to overwrite.`);
      process.exit(1);
    }
    warn('Existing Idea2PRD Manual installation found \u2014 overwriting (--force).');
  }
  ...
  // ── g) Success summary ─────────────────────────────────────────────────
  console.log('');
  success(bold('Idea2PRD Manual skill pack installed!'));
  console.log('');
  ...
  console.log(`  2. Run ${cyan('/idea2prd-manual [topic or question]')} to start analysis`);
  console.log(`  3. The skill orchestrates explore, research, and problem-solving`);
  console.log('');

  if (keysariumManifest) {
    console.log(bold('Integration:'));
    console.log(`  Idea2PRD Manual is available alongside your Keysarium pipeline.`);
    console.log(`  Use ${cyan('/casarium')} for research, ${cyan('/idea2prd-manual')} for deep analysis.`);
    console.log('');
  }
