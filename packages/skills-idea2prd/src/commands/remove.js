...
async function run(options) {
  ...
  if (!fileExists(manifestPath)) {
    warn('Idea2PRD Manual skill pack is not installed in this directory \u2014 nothing to remove.');
    process.exit(0);
  }
  ...
  console.log('');
  info(bold('The following Idea2PRD Manual components will be removed:'));
  console.log('');
  ...
  if (!force) {
    const confirmed = await confirmPrompt(
      yellow('This will remove all Idea2PRD Manual skill pack files. Continue? (y/N) ')
    );
  ...
  // ── f) Summary ────────────────────────────────────────────────────────
  console.log('');
  success(bold('Idea2PRD Manual skill pack removal complete!'));
  console.log(`  ${green('\u2713')} ${removedCount} file(s) removed`);
  if (skippedCount > 0) {
    console.log(`  ${dim('-')} ${skippedCount} file(s) already missing (skipped)`);
  }
  console.log('');
  info(`To reinstall, run: ${cyan('@dzhechkov/skills-idea2prd init')}`);
  console.log('');
