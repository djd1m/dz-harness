 ...
 * (unscoped) one.
 */
export function renderWindsurfMd(skill) {
    const description = skill.frontmatter.description.trim();
    // A skill MAY carry an explicit glob scope in its (loose) frontmatter; when
    // absent we omit `globs` entirely → a general, unscoped rule.
    const rawGlobs = skill.frontmatter.globs;
    ...
    // Windsurf honors `globs` ONLY under `trigger: glob`; under `model_decision` a `globs` field is
    // inert. So when a skill declares a glob scope, emit `trigger: glob` (globs are load-bearing);
    // otherwise `model_decision` (a general, agent-activated rule). This keeps the metadata coherent.
    const trigger = globs !== undefined ? 'glob' : 'model_decision';
    const front = ['---', `trigger: ${trigger}`, `description: ${yamlScalar(description)}`];
    if (globs !== undefined)
        front.push(`globs: ${yamlScalar(globs)}`);
    front.push('---');
    return `${front.join('\n')}\n\n${body}`;
}
