...
export const PLATFORMS = ['claude', 'codex', 'opencode', 'hermes', 'openclaude', 'copilot', 'agents-md', 'cursor', 'gemini', 'windsurf'] as const;
...
export type IntegrationReasonCode =
  ...
  | 'NO_QUALIFYING_LIVE_RECEIPT'
  | 'TARGET_BINARY_UNAVAILABLE'
  | 'RECEIPT_STALE'
  | 'PROJECT_CARRIER_NOT_OBSERVED'
  | 'LIVE_PROBE_FAILED'
  | 'CURRENT_LIVE_CHECK_FAILED'
  | 'LIVE_PROBE_TIMEOUT'
  | 'NO_ACTIVATION_RECEIPT'
  | 'PRODUCT_SURFACE_AMBIGUOUS'
  | 'PROJECT_CARRIER_NOT_DOCUMENTED'
  ...
  | 'APPLY_FAILED'
  | 'POST_WRITE_REGISTRATION_NOT_OBSERVED';
