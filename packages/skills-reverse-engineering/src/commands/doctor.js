...
function checkFilesExist(manifest, targetDir) {
  return check('Files exist', () => {
    ...
    return {
      ...
      fix: `Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to restore missing files.`,
    };
  });
}

function checkSkillAnalyst(targetDir) {
  return check('Reverse Engineering Unicorn Full skill', () => {
    const skillDir = path.join(targetDir, '.claude', 'skills', 'reverse-engineering-unicorn');

    if (!fileExists(skillDir)) {
      return {
        passed: false,
        detail: '.claude/skills/reverse-engineering-unicorn/ directory not found',
        fix: `Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to restore skill.`,
      };
    }
    ...
    if (!fileExists(skillMdPath)) {
      return {
        passed: false,
        detail: '.claude/skills/reverse-engineering-unicorn/SKILL.md not found',
        fix: `Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to restore SKILL.md.`,
      };
    }
...
function checkCommandPresent(targetDir) {
  return check('Reverse Engineering Unicorn command', () => {
    ...
    if (!fileExists(commandsDir)) {
      return {
        ...
        fix: `Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to restore command.`,
      };
    }
    ...
    const analystCommands = entries.filter(
      (e) => e.startsWith('reverse-engineering-unicorn') && e.endsWith('.md')
    );

    if (analystCommands.length === 0) {
      return {
        passed: false,
        detail: 'No Reverse Engineering Unicorn command files found (expected reverse-engineering-unicorn*.md)',
        fix: `Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to restore command.`,
      };
    }
...
function checkKeysariumIntegration(targetDir) {
  return check('Keysarium integration', () => {
    ...
    if (!fileExists(keysariumPath)) {
      return {
        passed: true,
        detail: 'Not installed (standalone Reverse Engineering Unicorn mode)',
      };
    }
...
async function run(options) {
  ...
  if (!fileExists(manifestPath)) {
    logError('Reverse Engineering Unicorn skill pack is not installed in this directory.');
    info(`Run ${cyan('@dzhechkov/skills-reverse-engineering init')} to install.`);
    process.exit(1);
  }
  ...
  console.log('');
  info(bold(`Running health checks for @dzhechkov/skills-reverse-engineering v${manifest.version}...`));
  console.log('');
  ...
  if (passedCount === total) {
    success(bold(`${passedCount}/${total} checks passed \u2014 Reverse Engineering Unicorn installation is healthy!`));
    console.log('');
    process.exit(0);
  }
