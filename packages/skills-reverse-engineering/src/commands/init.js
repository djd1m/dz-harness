...
function showKeysariumIntegration(keysariumManifest) {
  ...
  console.log(cyan('  \u2502') + ' Reverse Engineering Unicorn integrates with existing Keysarium.  ' + cyan('\u2502'));
...
async function run(options) {
  ...
  if (fileExists(manifestPath)) {
    if (!force) {
      warn('Reverse Engineering Unicorn skill pack is already installed in this directory.');
      info(`Run ${cyan('@dzhechkov/skills-reverse-engineering update')} to update, or use ${yellow('--force')} to overwrite.`);
      process.exit(1);
    }
    warn('Existing Reverse Engineering Unicorn installation found \u2014 overwriting (--force).');
  }
  ...
  // ── g) Success summary ─────────────────────────────────────────────────
  console.log('');
  success(bold('Reverse Engineering Unicorn skill pack installed!'));
  console.log('');
  ...
  console.log(`  2. Run ${cyan('/reverse-engineering-unicorn [topic or question]')} to start analysis`);
  console.log(`  3. The skill orchestrates explore, research, and problem-solving`);
  console.log('');

  if (keysariumManifest) {
    console.log(bold('Integration:'));
    console.log(`  Reverse Engineering Unicorn is available alongside your Keysarium pipeline.`);
    console.log(`  Use ${cyan('/casarium')} for research, ${cyan('/reverse-engineering-unicorn')} for deep analysis.`);
    console.log('');
  }
