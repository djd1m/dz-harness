...
async function run(options) {
  ...
  if (!fileExists(manifestPath)) {
    warn('Reverse Engineering Unicorn skill pack is not installed in this directory \u2014 nothing to remove.');
    process.exit(0);
  }
  ...
  console.log('');
  info(bold('The following Reverse Engineering Unicorn components will be removed:'));
  console.log('');
  ...
  if (!force) {
    const confirmed = await confirmPrompt(
      yellow('This will remove all Reverse Engineering Unicorn skill pack files. Continue? (y/N) ')
    );
  ...
  // ── f) Summary ────────────────────────────────────────────────────────
  console.log('');
  success(bold('Reverse Engineering Unicorn skill pack removal complete!'));
  console.log(`  ${green('\u2713')} ${removedCount} file(s) removed`);
  if (skippedCount > 0) {
    console.log(`  ${dim('-')} ${skippedCount} file(s) already missing (skipped)`);
  }
  console.log('');
  info(`To reinstall, run: ${cyan('@dzhechkov/skills-reverse-engineering init')}`);
  console.log('');
