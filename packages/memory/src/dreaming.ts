 ...
 */
const SYSTEM_WRAPPER_PREFIXES: readonly string[] = [
  '<task-notification',
  '<system-reminder',
 ...
 */
export function isSystemWrapper(text: string): boolean {
  const trimmed = text.trim();
  const lower = trimmed.toLowerCase();
  return (
    SYSTEM_WRAPPER_PREFIXES.some((prefix) => lower.startsWith(prefix)) ||
 ...
 */
const EXCELLENT_RESPONSES: ReadonlySet<string> = new Set([
  'ок',
  'ok',
  'next',
  'продолжай',
...
export function isNoiseInsight(text: string): boolean {
  ...
  if (text.startsWith(userPrefix)) {
    ...
    if (EXCELLENT_RESPONSES.has(normalizeCheckpointText(approval))) return true;
    return false;
  }
  return false;
}
    ...
    for (const line of lines) {
      try {
        ...
        if (entry.type === 'user' && typeof entry.message?.content === 'string') {
          const raw: string = entry.message.content;

          // System wrappers masquerade as user messages — skip entirely.
          if (isSystemWrapper(raw)) continue;
