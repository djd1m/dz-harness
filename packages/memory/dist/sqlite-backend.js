 ...
 * @internal
 */
export function searchPreparedRecords(stmts, hasFts5, query) {
    ...
    if (hasFts5 && query.text !== undefined && query.text.trim().length > 0) {
        try {
            ...
            if (ftsQuery.length > 0) {
                ...
                const ranked = rows
                    .map((row) => ({ record: rowToRecord(row), rank: row._rank }))
...
export class SqliteBackend {
     ...
     */
    static openReadOnly(filePath, opts) {
    ...
    put(record) {
        this.upsertStmt.run(record.id, record.skillId, record.text, record.score, record.outcome, record.timestamp, record.metadata ? JSON.stringify(record.metadata) : null);
