...
import { getSkillInfo, claimCheck, decideClaimCheckText, severityCounts, isGated, } from '@dzhechkov/harness-core';
 ...
 */
export function createDzMcpServer(options) {
    const { skillsDir } = options;
    const server = new McpServer({ name: 'dz-harness', version: MCP_SERVER_TOOLS_VERSION });
    server.registerTool('skill_list', { description: 'List the skills available in the harness skills directory.' }, () => jsonResult({ skillsDir, skills: listSkills(skillsDir) }));
    ...
    server.registerTool('skill_compile', {
    ...
    server.registerTool('harness_verify', {
    ...
    // --- New v0.7.0 tools ---
    server.registerTool('skill_info', {
        ...
        if (info === undefined)
            return errorResult(`skill not found: ${id}`);
        return jsonResult(info);
    });
    server.registerTool('preset_list', { description: 'List all available presets with their skill selections.' }, () => jsonResult({ presets: listPresets() }));
    server.registerTool('preset_get', {
        description: 'Get a preset by name, including its full skill list.',
        inputSchema: { name: z.string().describe('The preset name.') },
    }, ({ name }) => {
        const preset = getPreset(name);
        if (preset === undefined)
            return errorResult(`preset not found: ${name}`);
        return jsonResult(preset);
    });
    server.registerTool('claim_check_text', {
        description: 'Vet a block of PROSE for untagged quantitative/accuracy claims before you write it to disk. ' +
