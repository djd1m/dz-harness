...
Use this skill whenever the user wants to **clone, replicate, rebuild, reverse-engineer, or copy an existing website** ("make a copy of this site", "rebuild this page", "pixel-perfect clone"). It is the implementation counterpart to business reverse-engineering — it produces a *running* Next.js clone, not analysis. **In scope:** visual layout/styling, component structure & interactions, responsive design, mock data. **Out of scope:** real backend/auth/real-time, SEO, accessibility audit. **Requires** a browser-automation MCP and a Next.js/shadcn/Tailwind scaffold (see Pre-Flight).

## Procedure (overview)

Pre-Flight → **Phase 1** Reconnaissance → **Phase 2** Foundation → **Phase 3** Component Specification & Dispatch → **Phase 4** Page Assembly → **Phase 5** Visual QA Diff → Output & Completion. Extraction and construction run in parallel (the "foreman" model below).
...
## Anti-Patterns / Self-Check (What NOT to Do)
