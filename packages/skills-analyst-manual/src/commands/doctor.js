...
function checkFilesExist(manifest, targetDir) {
  return check('Files exist', () => {
    ...
    return {
      ...
      fix: `Run ${cyan('@dzhechkov/skills-analyst-manual update')} to restore missing files.`,
    };
  });
}

function checkSkillAnalyst(targetDir) {
  return check('Analyst Manual Full skill', () => {
    const skillDir = path.join(targetDir, '.claude', 'skills', 'analyst-manual-full');

    if (!fileExists(skillDir)) {
      return {
        passed: false,
        detail: '.claude/skills/analyst-manual-full/ directory not found',
        fix: `Run ${cyan('@dzhechkov/skills-analyst-manual update')} to restore skill.`,
      };
    }
    ...
    if (!fileExists(skillMdPath)) {
      return {
        passed: false,
        detail: '.claude/skills/analyst-manual-full/SKILL.md not found',
        fix: `Run ${cyan('@dzhechkov/skills-analyst-manual update')} to restore SKILL.md.`,
      };
    }
...
function checkCommandPresent(targetDir) {
  return check('Analyst Manual command', () => {
    ...
    if (!fileExists(commandsDir)) {
      return {
        ...
        fix: `Run ${cyan('@dzhechkov/skills-analyst-manual update')} to restore command.`,
      };
    }
    ...
    const analystCommands = entries.filter(
      (e) => e.startsWith('analyst-manual') && e.endsWith('.md')
    );

    if (analystCommands.length === 0) {
      return {
        passed: false,
        detail: 'No Analyst Manual command files found (expected analyst-manual*.md)',
        fix: `Run ${cyan('@dzhechkov/skills-analyst-manual update')} to restore command.`,
      };
    }
...
function checkKeysariumIntegration(targetDir) {
  return check('Keysarium integration', () => {
    ...
    if (!fileExists(keysariumPath)) {
      return {
        passed: true,
        detail: 'Not installed (standalone Analyst Manual mode)',
      };
    }
...
async function run(options) {
  ...
  console.log('');
  info(bold(`Running health checks for @dzhechkov/skills-analyst-manual v${manifest.version}...`));
  console.log('');
  ...
  if (passedCount === total) {
    success(bold(`${passedCount}/${total} checks passed \u2014 Analyst Manual installation is healthy!`));
    console.log('');
    process.exit(0);
  }
