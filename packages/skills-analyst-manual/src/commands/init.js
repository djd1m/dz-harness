...
function showKeysariumIntegration(keysariumManifest) {
  ...
  console.log(cyan('  \u2502') + ' Analyst Manual integrates with existing Keysarium.  ' + cyan('\u2502'));
...
async function run(options) {
  ...
  if (fileExists(manifestPath)) {
    if (!force) {
      warn('Analyst Manual skill pack is already installed in this directory.');
      info(`Run ${cyan('@dzhechkov/skills-analyst-manual update')} to update, or use ${yellow('--force')} to overwrite.`);
      process.exit(1);
    }
    warn('Existing Analyst Manual installation found \u2014 overwriting (--force).');
  }
  ...
  // ── g) Success summary ─────────────────────────────────────────────────
  console.log('');
  success(bold('Analyst Manual skill pack installed!'));
  console.log('');
  ...
  console.log(`  2. Run ${cyan('/analyst-manual [topic or question]')} to start analysis`);
  console.log(`  3. The skill orchestrates explore, research, and problem-solving`);
  console.log('');

  if (keysariumManifest) {
    console.log(bold('Integration:'));
    console.log(`  Analyst Manual is available alongside your Keysarium pipeline.`);
    console.log(`  Use ${cyan('/casarium')} for research, ${cyan('/analyst-manual')} for deep analysis.`);
    console.log('');
  }
