...
export type Finding = { kind: FindingKind; line?: number; file?: string; detail: string };
export type MemoryIndexReport = { bytes: number; lines: number; findings: readonly Finding[] };
...
export function checkMemoryIndex(input: {
  ...
  limits?: { maxBytes?: number; maxLineChars?: number; minHookSupport?: number };
}): MemoryIndexReport {
  ...
  for (const file of [...files.keys()].sort()) {
    if (!indexed.has(file)) findings.push({ kind: 'unindexed-file', file, detail: 'file has no index entry' });
  }
