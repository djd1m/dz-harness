 ...
 */
export function withProjectLockSync<T>(
  projectRoot: string,
  name: string,
  fn: () => T,
  ...
  if (!storeExists(projectRoot)) throw new StoreAbsentError(projectRoot, name);
...
function withLockSync<T>(lockPath: string, name: string, fn: () => T, opts: StoreLockOptions): T {
