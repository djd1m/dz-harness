...
// ── The template vocabulary (ADR-002) ───────────────────────────────────────────────────────────
...
export function validTemplateParams(template: unknown, params: unknown): params is TemplateParams {
  if (typeof template !== 'string' || !TEMPLATES.includes(template as RuleTemplate)) return false;
  if (!params || typeof params !== 'object' || Array.isArray(params)) return false;
  const p = params as Record<string, unknown>;
  const str = (k: string): boolean => typeof p[k] === 'string' && (p[k] as string).length > 0 && (p[k] as string).length <= MAX_GLOB_LENGTH;
  ...
  const glob = (k: string): boolean => str(k) && isSafeGlob(p[k]);
  if (template === 'pairing-check') return glob('when') && glob('requires');
  if (template === 'absence-check') return glob('forbid');
...
export function globWildcardCount(glob: unknown): number {
...
export function isSafeGlob(glob: unknown): glob is string {
  return typeof glob === 'string' && glob.length > 0 && glob.length <= MAX_GLOB_LENGTH && globWildcardCount(glob) <= MAX_GLOB_WILDCARDS;
}

export function globMatch(glob: unknown, path: unknown): boolean {
  if (typeof glob !== 'string' || typeof path !== 'string' || glob === '') return false;
...
// ── The ONE predicate — shared by shadow replay and the live guard checker ───────────────────────

export type FireOutcome = { readonly fired: boolean; readonly detail?: string } | { readonly undecidable: string };
 ...
 */
export function templateFires(template: RuleTemplate, params: TemplateParams, change: ChangeSet): FireOutcome {
  const files = Array.isArray(change?.files) ? change.files.filter((f): f is string => typeof f === 'string') : [];
  if (template === 'pairing-check') {
    const armed = files.filter((f) => globMatch(params.when, f));
    if (armed.length === 0) return { fired: false };
    if (files.some((f) => globMatch(params.requires, f))) return { fired: false };
  ...
  if (!contents || typeof contents !== 'object') return { undecidable: `no content for ${targets.length} file(s) matching ${params.file}` };
  const missing: string[] = [];
  for (const t of targets) {
    const text = Object.hasOwn(contents, t) ? contents[t] : undefined;
    if (typeof text !== 'string') return { undecidable: `no content for ${t}` };
    if (!text.includes(String(params.mustMatch))) missing.push(t);
  }
...
// ── Classification: lesson text → (template, params), or an honest refusal ───────────────────────
...
export function isClassified(x: ClassifiedLesson | ClassifyRefusal): x is ClassifiedLesson {
...
function backticked(text: string): string[] {
  ...
  while ((m = re.exec(text)) !== null) if (m[1] !== undefined) out.push(m[1].trim());
  return out;
}

/** Artifact tokens in TEXT ORDER, deduped. A token with no `/` becomes a basename glob. */
export function artifactTokens(text: string): string[] {
...
export function tokenToGlob(token: string): string {
 ...
 */
export function classifyLesson(text: unknown): ClassifiedLesson | ClassifyRefusal {
  if (typeof text !== 'string' || text.trim() === '') return { reason: 'not-promotable: empty lesson text' };
  ...
  if (PAIRING_RE.test(text)) {
    if (tokens.length < 2) return { reason: `not-promotable: pairing-shaped but only ${tokens.length} artifact token(s) — cannot bind when/requires` };
    if (tokens.length > 3) return { reason: `not-promotable: pairing-shaped but ambiguous (${tokens.length} distinct artifact tokens; the classifier binds at most 2)` };
    const when = tokens[0]!;
  ...
  if (ABSENCE_RE.test(text)) {
    if (tokens.length !== 1) return { reason: `not-promotable: absence-shaped but ${tokens.length} artifact token(s) — absence-check binds exactly 1` };
  ...
  if (FORMAT_RE.test(text)) {
    ...
    if (tokens.length !== 1) return { reason: `not-promotable: format-shaped but ${tokens.length} artifact token(s) — format-match binds exactly 1` };
    if (literals.length !== 1) return { reason: `not-promotable: format-shaped but ${literals.length} backticked literal(s) — format-match needs exactly 1 to match against` };
 ...
 */
export function derivedRuleId(c: ClassifiedLesson): string {
  const slug = c.tokens
 ...
 */
export function isSafeRuleId(id: unknown): id is string {
 ...
 */
export function promotionAdrRelPath(ruleId: unknown, seq: unknown): string | null {
  if (!isSafeRuleId(ruleId)) return null;
  if (typeof seq !== 'number' || !Number.isInteger(seq) || seq < 0 || seq > 100_000) return null;
 ...
 */
export const BUILTIN_COVERAGE: Readonly<Record<string, { readonly template: RuleTemplate; readonly params: TemplateParams }>> = {
...
export function paramsKey(template: RuleTemplate, params: TemplateParams): string {
  const p = params ?? {};
  const parts = Object.keys(p)
    .sort()
    .map((k) => `${k}=${String((p as Record<string, unknown>)[k]).trim()}`);
...
export function coveringRule(c: ClassifiedLesson, existing: readonly ExistingRuleView[]): string | null {
  const key = paramsKey(c.template, c.params);
  for (const [id, cov] of Object.entries(BUILTIN_COVERAGE)) {
    if (paramsKey(cov.template, cov.params) === key && existing.some((e) => e?.id === id)) return id;
  }
  for (const e of existing) {
    if (!e || typeof e.id !== 'string') continue;
    if (e.template !== undefined && e.params !== undefined && paramsKey(e.template, e.params) === key) return e.id;
  }
  return null;
}
 ...
 */
export function evaluateCandidate(c: ClassifiedLesson, periods: readonly Period[], minChanges: number = MIN_CHANGES_PER_PERIOD): CandidateEvaluation {
  ...
  for (const p of Array.isArray(periods) ? periods : []) {
    ...
    for (const ch of changes) {
      ...
      if (Object.hasOwn(r, 'undecidable')) {
        undecidable = undecidable ?? (r as { undecidable: string }).undecidable;
        continue;
      }
      if ((r as { fired: boolean }).fired) {
        firings += 1;
        evidence = evidence ?? `${ch.id}${(r as { detail?: string }).detail ? `: ${(r as { detail?: string }).detail}` : ''}`;
      }
    }
    evaluated += 1;
...
export function promotedRuleObject(c: ClassifiedLesson, ruleId: string, lessonId: string): PromotedRule {
  const what =
    c.template === 'pairing-check'
 ...
 */
export function assembleCandidates(facts: PromotionFacts): PromotionReport {
  ...
  const windowDays = Number.isFinite(facts?.windowDays) ? (facts.windowDays as number) : DEFAULT_WINDOW_DAYS;
  const periodCount = Number.isFinite(facts?.periods) ? (facts.periods as number) : DEFAULT_PERIODS;
  ...
  for (const l of lessons) {
    ...
    const firstSeenTs = Number.isFinite(firstSeenMs) ? (firstSeenRaw as string) : null;
    ...
    if (ev.wins >= WINS_TO_PROMOTE) {
      ...
      if (elapsedMs < elapsedRequiredMs) {
        const days = (ms: number): string => (ms / DAY_MS).toFixed(1);
        out.push({
          ...common,
          verdict: 'wait',
          reason:
            `wait: ${ev.wins} consecutive shadow win(s), but only ${days(elapsedMs)}d of the ${days(elapsedRequiredMs)}d REAL elapsed time required since first observation` +
            (firstSeenTs === null
  ...
  const n = (v: CandidateVerdict): number => out.filter((c) => c.verdict === v).length;
 ...
 */
export function normalizePromotionState(raw: unknown): PromotionState {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return EMPTY_PROMOTION_STATE;
  const o = raw as Record<string, unknown>;
  if (!Object.hasOwn(o, 'version') || o['version'] !== 1) return EMPTY_PROMOTION_STATE;
  ...
  if (rawEntries && typeof rawEntries === 'object' && !Array.isArray(rawEntries)) {
    for (const key of Object.keys(rawEntries as Record<string, unknown>)) {
      if (!Object.hasOwn(rawEntries as object, key) || UNSAFE_KEYS.has(key)) continue;
      const e = (rawEntries as Record<string, unknown>)[key];
      if (!e || typeof e !== 'object' || Array.isArray(e)) continue;
      const r = e as Record<string, unknown>;
      const str = (k: string): string | undefined => (typeof r[k] === 'string' ? (r[k] as string) : undefined);
      const int = (k: string): number => (typeof r[k] === 'number' && Number.isInteger(r[k]) && (r[k] as number) >= 0 ? (r[k] as number) : 0);
      const ruleId = str('ruleId');
      const lessonId = str('lessonId');
      if (ruleId === undefined || lessonId === undefined) continue;
      ...
      if (!isSafeRuleId(key) || key !== ruleId) continue;
 ...
 */
export function nextPromotionState(prev: PromotionState, report: PromotionReport, nowTs: string, adrSeqs: Readonly<Record<string, number>> = {}, newlyAllocated?: number): PromotionState {
  const base = normalizePromotionState(prev);
  const entries: Record<string, PromotionStateEntry> = { ...base.entries };
  for (const c of report.candidates) {
    ...
    if (c.ruleId === null || UNSAFE_KEYS.has(c.ruleId) || !isSafeRuleId(c.ruleId)) continue;
...
const GLYPH: Record<CandidateVerdict, string> = {
  promote: '★',
  wait: '·',
  'insufficient-data': '?',
  duplicate: '=',
...
export function renderPromotionReport(r: PromotionReport, limit = 15): string {
  const out: string[] = [];
  ...
  if (ranked.length === 0) {
    ...
    for (const c of ranked.slice(0, limit)) {
      ...
      if (c.periods.length > 0) {
        ...
        if (ev?.evidence !== undefined) out.push(`        evidence: ${ev.evidence}`);
      }
    }
    if (ranked.length > limit) out.push(`    … ${ranked.length - limit} more`);
  }
  ...
  if (refused.length > 0) {
    ...
    for (const [reason, n] of [...byReason.entries()].sort((a, b) => b[1] - a[1])) out.push(`    ${String(n).padStart(4)} × ${reason}`);
  }
  out.push('');
 ...
 */
export function renderPromotionAdr(c: PromotionCandidate, seq: number, nowTs: string): string {
  ...
  if (c.periods.length > 0) {
    ...
    out.push('|---|---|---|---|---|---|');
    for (const p of c.periods) out.push(`| ${p.start} | ${p.end} | ${p.changes} | ${p.firings} | ${p.outcome} | ${p.evidence ?? '—'} |`);
  }
  out.push('');
  out.push('## Reason');
  out.push('');
