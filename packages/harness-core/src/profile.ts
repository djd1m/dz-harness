...
export const REGISTERS: readonly Register[] = ['pro', 'pro-lite', 'plain'];
...
const REGISTER_ALIASES: Record<string, Register> = {
  'pro': 'pro',
  'профи': 'pro',
  'expert': 'pro',
  'pro lite': 'pro-lite',
...
export function registerOwnerWord(register: Register): string {
  switch (register) {
 ...
 * silently defaults. */
export function parseRegister(raw: string): Register | null {
 ...
 * Never throws.
 */
export function parseDomainList(raw: string): Domain[] {
  ...
  for (const ch of raw) {
    ...
    if (ch === ',' && depth === 0) { pieces.push(current); current = ''; }
    else current += ch;
  }
  pieces.push(current);
  const out: Domain[] = [];
  for (const piece of pieces) {
    ...
    if (tag === '') continue; // "(CCIE)" alone names nothing — skip, never throw
...
export function domainListText(domains: readonly Domain[]): string {
...
export function parseYesNo(raw: string): boolean | null {
  ...
  if (['y', 'yes', 'да', 'true', 'on'].includes(t)) return true;
  if (['n', 'no', 'нет', 'false', 'off'].includes(t)) return false;
  return null;
}
...
export function profileStorePath(home?: string): string {
...
function validDomainList(v: unknown): Domain[] | null {
...
export function validateProfile(v: unknown): { profile: OperatorProfile | null; problem: string | null } {
  try {
...
function validateProfileShape(v: unknown): { profile: OperatorProfile | null; problem: string | null } {
  if (v === null || typeof v !== 'object') return { profile: null, problem: 'invalid shape: not an object' };
  const o = v as Record<string, unknown>;
  if (o.version !== 1) return { profile: null, problem: `invalid shape: version must be 1, got ${JSON.stringify(o.version)}` };
  if (typeof o.updatedAt !== 'string') return { profile: null, problem: 'invalid shape: updatedAt must be a string' };
  if (typeof o.language !== 'string' || o.language.trim() === '') return { profile: null, problem: 'invalid shape: language must be a non-empty string' };
  ...
  if (deep === null) return { profile: null, problem: 'invalid shape: deepDomains must be an array of { tag, note? }' };
  const weak = validDomainList(o.weakDomains);
  if (weak === null) return { profile: null, problem: 'invalid shape: weakDomains must be an array of { tag, note? }' };
  if (typeof o.teaches !== 'boolean') return { profile: null, problem: 'invalid shape: teaches must be a boolean' };
  ...
  const fieldStrings: string[] = [o.updatedAt as string, o.language as string,
...
export function readProfile(home?: string): ProfileReadResult {
  const path = profileStorePath(home);
  if (!existsSync(path)) return { profile: null, path, problem: 'missing' };
  let raw: string;
  try {
...
export function writeProfile(profile: OperatorProfile, home?: string): { path: string; problem?: string } {
  ...
  if (ok === null) return { path: profileStorePath(home), problem: problem ?? 'invalid profile' };
  const path = profileStorePath(home);
...
function domainLine(domains: readonly Domain[]): string {
  return domains.map((d) => (d.note ? `${d.tag} (${d.note})` : d.tag)).join(' · ');
}

function registerLine(register: Register): string {
  switch (register) {
    case 'pro':
...
export function renderProfileBlock(profile: OperatorProfile): string {
  ...
  if (profile.deepDomains.length > 0) {
    parts.push(
      `**Deep domains — go full pro here, no scaffolding:**\n${domainLine(profile.deepDomains)}.\n` +
  ...
  if (profile.weakDomains.length > 0) {
    parts.push(
      `**Weak domains — one plain sentence EVERY time, unprompted, without waiting to be asked:**\n${domainLine(profile.weakDomains)}.\n` +
  ...
  parts.push(
    '**An explanation is SELF-CONTAINED.** Every term introduced gets its gloss at first use IN THIS PASSAGE, ' +
  ...
  if (profile.teaches) {
    parts.push(
      '**The operator teaches — explanations must be RE-TELLABLE.** Give the formulation they could carry into a ' +
  ...
  parts.push(
    '**The shape of a good pro-lite explanation:**\n' +
  ...
  parts.push(
    '**Scope of the register.** It governs DIALOGUE and owner-facing surfaces: answers, checkpoint questions, ' +
  ...
  parts.push(
    '**The register changes FORM, never the FACTS.** Numbers, caveats, risks and refutations survive every level. ' +
    'Simplifying is not licence to drop a measurement or soften a bad result.',
  );
  parts.push(
    '**Escape hatches, effective immediately for the session:** «проще» / «подробнее» / «как профи» / «я это знаю». ' +
...
export function wrapProfileBlock(content: string): string {
...
export function extractProfileBlock(fileContent: string): string | null {
...
export function mergeProfileBlock(existing: string | null, content: string): { merged: string; changed: boolean; hadBlock: boolean; malformed?: boolean; malformedKind?: 'dangling-start' | 'nested-markers' } {
...
export function syncProfileBlock(profile: OperatorProfile, home?: string, now?: Date): ProfileSyncResult {
  ...
  try {
    ...
    if (!merge.changed) return { target, changed: false, backup: null, problem: null };
    ...
    if (existing === null) {
      ...
      if (tightened !== prevMode) chmodSync(target, tightened);
    }
...
export function checkProfileDrift(home?: string): ProfileDriftResult {
  ...
  const target = claudeMdPath(home);
  if (!existsSync(target)) return { verdict: 'no-target', detail: `${target} does not exist — run \`dz profile sync\`` };
  ...
  if (inFile === expected) return { verdict: 'in-sync', detail: 'block matches profile.json' };
...
export function profileAgeDays(profile: OperatorProfile, now?: Date): number | null {
