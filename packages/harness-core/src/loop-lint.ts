...
interface Ctx {
  script: string;
  /** script with comments stripped (ban scans ignore prose). */
  code: string;
  lines: string[];
  plan: LoopPlan | null;
  projection: LintProjection | null;
...
function noPlan(rule: string): LintFinding[] {
  return [{ ...NO_PLAN, rule }];
}

// ── rule implementations (each returns findings; empty = pass) ───────────────

function ruleMetaComplete(ctx: Ctx): LintFinding[] {
...
function metaPhaseTitles(script: string): string[] {
  ...
  return [...seg.matchAll(/title:\s*['"]([^'"]+)['"]/g)].map((m) => m[1] as string);
}

function rulePhaseParity(ctx: Ctx): LintFinding[] {
  const titles = metaPhaseTitles(ctx.script);
  const calls: string[] = [];
  for (const m of ctx.code.matchAll(/(?<![.\w])phase\(\s*['"]([^'"]+)['"]\s*\)/g)) {
    const p = m[1] as string;
    if (!calls.includes(p)) calls.push(p);
  }
  if (titles.length === 0 || calls.length === 0) return []; // meta-complete owns the missing-block case
...
const SANDBOX_BANS = ['Date.now(', 'new Date', 'Math.random(', 'require(', 'process.', 'child_process'] as const;

function ruleSandboxBans(ctx: Ctx): LintFinding[] {
  const out: LintFinding[] = [];
...
function ruleShqHygiene(ctx: Ctx): LintFinding[] {
...
function callArguments(code: string, openIdx: number): string[] {
  const args: string[] = [];
  let depth = 1;
  let cur = '';
  let i = openIdx;
...
function ruleAgentLabelled(ctx: Ctx): LintFinding[] {
  const out: LintFinding[] = [];
  let n = 0;
  for (const m of ctx.code.matchAll(/(?<![.\w])agent\(/g)) {
    ...
    if (opts === undefined) {
      ...
      if (/\.(replace|match|split|test|exec)\s*\(\s*\//.test(consumed)) continue;
      bad = 'no opts argument at all';
    } else if (opts.startsWith('{')) {
      // inline object literal — label + phase must be present in it
      if (!/label\s*:/.test(opts)) bad = 'no label in opts';
      else if (!/phase\s*:/.test(opts)) bad = 'no phase in opts';
    }
  ...
  if (n > 3) out.push({ rule: 'agent-labelled', severity: 'fail', message: `…and ${n - 3} more unlabelled agent() call(s)` });
  return out;
}

function ruleNoPartialCheckpoint(ctx: Ctx): LintFinding[] {
  const out: LintFinding[] = [];
  ...
  for (const m of ctx.code.matchAll(/catch\s*(?:\(([^)]*)\))?\s*\{\s*\}/g)) {
    out.push({ rule: 'no-partial-checkpoint', severity: 'warn', message: 'a catch block that neither logs nor rethrows swallows a failure silently (advisory)', anchor: (m[0] as string).slice(0, 40) });
    break;
  }
  return out;
}
...
function rulePlanBinding(ctx: Ctx, digestOfPlan: string | null): LintFinding[] {
  const m = HEADER_RE.exec(ctx.script);
  if (ctx.plan === null) return noPlan('plan-binding');
  if (!m) return [{ rule: 'plan-binding', severity: 'fail', message: 'plan given but the script carries no LOOP-PLAN header line', anchor: 'LOOP-PLAN' }];
  if (digestOfPlan !== null && m[1] !== digestOfPlan) {
    return [{ rule: 'plan-binding', severity: 'fail', message: `header digest sha256:${(m[1] as string).slice(0, 12)}… does not match planDigest sha256:${digestOfPlan.slice(0, 12)}… — the script was rendered from a DIFFERENT plan`, anchor: 'LOOP-PLAN' }];
  }
  return [];
}
...
function ruleBlobHash(ctx: Ctx): LintFinding[] {
  const out: LintFinding[] = [];
  for (const m of ctx.script.matchAll(BLOB_RE)) {
    const [, name, version, declaredHash, , body] = m as unknown as [string, string, string, string, string, string];
...
function resolveAssignChain(ctx: Ctx, name: string, hops: number): string {
  if (hops <= 0) return '';
  const m = new RegExp(`(?:const|let|var)\\s+${name}\\s*=\\s*([^\\n]+)`).exec(ctx.code);
  if (!m) return '';
  const rhs = m[1] as string;
  const base = /^([A-Za-z_$][\w$]*)/.exec(rhs.trim());
  const deeper = base && base[1] !== name ? resolveAssignChain(ctx, base[1] as string, hops - 1) : '';
  return rhs + '\n' + deeper;
}

function ruleFanoutBounded(ctx: Ctx): LintFinding[] {
  const out: LintFinding[] = [];
  if (ctx.projection) {
  ...
  for (const m of ctx.code.matchAll(/parallel\(\s*([A-Za-z_$][\w$]*)\s*[),]/g)) {
    ...
    if (!chain.includes('.map(')) continue;
  ...
  for (const m of ctx.code.matchAll(/parallel\(\s*([A-Za-z_$][\w$]*(?:\.[\w$]+)*)\.map\(/g)) {
    const base = (m[1] as string).split('.')[0] as string;
...
function successors(projection: LintProjection): Map<string, string[]> {
  const succ = new Map<string, string[]>();
  for (const n of projection.nodes) succ.set(n, []);
...
export function postDominators(projection: LintProjection): Map<string, Set<string>> {
  const succ = successors(projection);
  const nodes = projection.nodes;
  const pdom = new Map<string, Set<string>>();
  for (const n of nodes) pdom.set(n, n === 'exit' ? new Set(['exit']) : new Set(nodes));
  let changed = true;
  while (changed) {
    changed = false;
    for (const n of nodes) {
      ...
      for (const s of succs) {
        const sd = pdom.get(s) ?? new Set<string>();
        if (inter === null) {
          inter = new Set(sd);
        } else {
          const cur: Set<string> = inter;
          inter = new Set([...cur].filter((x) => sd.has(x)));
        }
      }
      const next = new Set(inter ?? []);
      next.add(n);
      const prev = pdom.get(n) ?? new Set<string>();
...
export function dominators(projection: LintProjection): Map<string, Set<string>> {
  const pred = new Map<string, string[]>();
  for (const n of projection.nodes) pred.set(n, []);
  for (const e of projection.edges) {
  ...
  while (changed) {
    ...
    for (const n of nodes) {
      ...
      for (const p of preds) {
        const pd = dom.get(p) ?? new Set<string>();
        if (inter === null) {
          inter = new Set(pd);
        } else {
          const cur: Set<string> = inter;
      ...
      const next = new Set(inter ?? []);
      next.add(n);
      const prev = dom.get(n) ?? new Set<string>();
      if (next.size !== prev.size || [...next].some((x) => !prev.has(x))) {
        dom.set(n, next);
        changed = true;
      }
...
function ruleBarrierPostdominates(ctx: Ctx): LintFinding[] {
  if (ctx.projection === null) return noPlan('barrier-postdominates');
  const out: LintFinding[] = [];
  ...
  for (const pair of ctx.projection.forkJoinPairs) {
    ...
    const policyOk = (JOIN_POLICIES as readonly string[]).includes(pair.policy) || QUORUM_RE.test(pair.policy);
    if (!policyOk) {
    ...
    const forkPdom = pdom.get(pair.fork) ?? new Set<string>();
    ...
    for (const b of pair.branches) {
      const bPdom = pdom.get(b) ?? new Set<string>();
...
function ruleBudgetBeforeSpawn(ctx: Ctx): LintFinding[] {
  if (ctx.projection === null) return noPlan('budget-before-spawn');
  const out: LintFinding[] = [];
...
function ruleRetryIdempotent(ctx: Ctx): LintFinding[] {
  if (ctx.projection === null) return noPlan('retry-idempotent');
  const out: LintFinding[] = [];
  for (const f of ctx.projection.facts) {
...
function ruleResumeFingerprint(ctx: Ctx): LintFinding[] {
...
function rulePauseWired(ctx: Ctx): LintFinding[] {
  if (ctx.projection === null) return noPlan('pause-wired');
  const out: LintFinding[] = [];
  for (const p of ctx.projection.pauses) {
    ...
    if (!returned) out.push({ rule: 'pause-wired', severity: 'fail', message: `declared pause state "${p.state}" is returned nowhere in the script (INV-5)`, anchor: p.state });
    if (!resumeRead) out.push({ rule: 'pause-wired', severity: 'fail', message: `pause "${p.state}": resumeArg "${p.resumeArg}" is read nowhere — a re-invoke could never resume`, anchor: p.resumeArg });
  }
  return out;
}

function ruleDispatchByDeliverable(ctx: Ctx): LintFinding[] {
  if (ctx.projection === null) {
...
function ruleNoAgentOutsideRunstep(ctx: Ctx): LintFinding[] {
  ...
  for (const m of ctx.script.matchAll(/(?<![.\w])agent\(/g)) {
    const idx = m.index ?? 0;
    if (idx >= start && idx <= end) continue; // inside runStep
    ...
    if (line.includes('loop-lint: infra-agent')) continue; // marked infra (trace flush / ckpt write)
    if (line.includes('runStep(')) continue; // a thunk handed TO runStep — the choke point wraps it
    const commentAt = line.indexOf('//');
    if (commentAt !== -1 && lineStart + commentAt < idx) continue; // prose mention in a comment
...
export function lint(scriptText: string, opts: LintOptions = {}): LintRun {
  const mode: LintMode = opts.mode ?? 'default';
  ...
  const run = (id: LintRuleId, fn: () => LintFinding[]): void => {
    const fs = fn();
    let sev: LintSeverity = 'pass';
    for (const f of fs) {
      findings.push(f);
      if (f.severity === 'fail') sev = 'fail';
      else if (f.severity === 'inconclusive' && sev !== 'fail') sev = 'inconclusive';
      else if (f.severity === 'warn' && sev === 'pass') sev = 'warn';
    }
    rules[id] = sev;
  };
  ...
  let verdict: LintVerdict;
  if (severities.includes('fail')) verdict = 'fail';
...
export function lintExitCode(run: LintRun): number {
