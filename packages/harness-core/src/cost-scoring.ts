 ...
 */
function priced(prompt: number, completion: number, cacheReadFactor = 0.1): ModelPricing {
  return {
    prompt,
    completion,
 ...
 */
export const MODEL_PRICES: Readonly<Record<string, ModelPricing>> = {
...
export function usageCost(usage: NormalizedUsage, modelId: string): number {
  const p = pricingFor(modelId);
  return (
    usage.promptTokens * p.prompt +
...
export function invocationCost(records: readonly CallRecord[]): InvocationCost {
 ...
 */
export function costEfficiency(qualityScore: number, costUsd: number): number {
  if (costUsd <= 0) return qualityScore > 0 ? Infinity : 0;
  return qualityScore / costUsd;
}

// ── Static cost estimation (from SKILL.md content) ──────────
...
const TIER_UNITS = { haiku: 1, sonnet: 4, opus: 20 } as const;
 ...
 */
export function estimateSkillCost(skillContent: string): CostEstimate {
  const lower = skillContent.toLowerCase();
  const count = (re: RegExp): number => (lower.match(re) ?? []).length;
  ...
  const relativeUnits =
    tiers.haiku * TIER_UNITS.haiku +
    tiers.sonnet * TIER_UNITS.sonnet +
    tiers.opus * TIER_UNITS.opus;

  let level: CostEstimate['level'];
  if (relativeUnits === 0) level = 'free';
  else if (relativeUnits <= 8) level = 'low';
  else if (relativeUnits <= 40) level = 'medium';
  else level = 'high';
