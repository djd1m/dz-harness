...
import { recordPattern, recallPatterns, type PatternRecord } from '../patterns.js';
...
async function seed(): Promise<void> {
  const rows: PatternRecord[] = Array.from({ length: 24 }, (_, i) => ({
  ...
  for (const r of rows) await recordPattern(project, r);
}
