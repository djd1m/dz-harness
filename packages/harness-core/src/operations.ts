 ...
 */
function enrichEmitForTarget(
  emit: EmitResult,
  target: TargetName,
  skillId: string,
  skill: CanonicalSkill,
): EmitResult {
  const extra: SkillAsset[] = [];
...
// ---------------------------------------------------------------------------
// Single-file, flattening aggregation across ALL selected skills
// (agents-md → AGENTS.md, gemini → GEMINI.md — same shape, different filename)
// ---------------------------------------------------------------------------
 ...
 */
const AGENTS_MD_LOSSY_WARNING =
  `agents-md is a lossy target: every selected skill is flattened into a plain-Markdown ` +
 ...
 * the lossless source.
 */
const GEMINI_MD_LOSSY_WARNING =
  `gemini is a lossy target: every selected skill is flattened into a plain-Markdown ` +
 ...
 */
function runInitSingleFileMd(options: AgentsMdInitOptions, config: SingleFileMdConfig): InitReport {
  ...
  const picked: { id: string; skillsDir: string }[] = [];
 ...
 */
export function runInitAgentsMd(options: AgentsMdInitOptions): InitReport {
 ...
 */
export function runInitGeminiMd(options: AgentsMdInitOptions): InitReport {
  return runInitSingleFileMd(options, {
    target: 'gemini',
...
export async function runDoctor(options: { projectRoot: string }): Promise<DoctorReport> {
  ...
  if (existsSync(configPath)) {
    try {
      ...
      const rawProject: unknown = parsed?.project;
      const projectName =
        typeof rawProject === 'string'
          ? rawProject
          : rawProject && typeof rawProject === 'object' && typeof (rawProject as { name?: unknown }).name === 'string'
            ? (rawProject as { name: string }).name
            : null;
      checks.push({
