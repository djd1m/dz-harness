 ...
 */
export function needsLandedBarrier(opts: StageOpts | null | undefined): boolean {
...
const CODE_LANDING_PIPELINE_PREFIXES = ['features/', '.dz/', '.agentic-qe/', 'roam/'] as const;
...
function codeLandingEmptySignal(seconds: number): string {
  return 'changed=0 after ' + seconds + 's — genuinely not landed';
}

export function needsCodeLandedBarrier(coderUsed: string | null | undefined): boolean {
 ...
 */
export function modelLabel(opts: StageOpts | null | undefined): string {
 ...
 */
export function stageLabel(base: string, opts: StageOpts | null | undefined): string {
...
export const CODEX_QE_DECLINE_KINDS: readonly string[] = ['timeout', 'no-verdict', 'tool-error', 'unusable-output', 'unavailable', 'over-ceiling', 'wrong-tree', 'scope-not-established', 'base-ref-not-established'];
