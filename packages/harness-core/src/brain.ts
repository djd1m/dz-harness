...
function defaultRegistry(): BrainRegistry {
 ...
 */
export function readRegistry(home: string): BrainRegistry {
  ...
  try {
    ...
    if (
      ...
      return { version: 1, sources: (parsed as BrainRegistry).sources };
    }
 ...
 */
export function writeRegistry(home: string, reg: BrainRegistry): void {
  try {
...
export function listBrain(home?: string): BrainSource[] {
...
interface NativeDb {
  ...
  prepare: (q: string) => { run: (...a: unknown[]) => unknown; all: (...a: unknown[]) => unknown[] };
  close: () => void;
}

/** Resolve the project's `better-sqlite3` from `depsRoot` (matches the reused primitives). */
function resolveSqlite(depsRoot: string): { url: string } | { error: string } {
  try {
 ...
 */
async function deleteBrainVectors(depsRoot: string, agentdbPath: string, book: string): Promise<void> {
  if (!existsSync(agentdbPath)) return; // nothing indexed yet
  const sqlite = resolveSqlite(depsRoot);
  if ('error' in sqlite) return;
  try {
    const { default: Database } = (await import(sqlite.url)) as { default: new (p: string) => NativeDb };
    const db = new Database(agentdbPath);
    try {
      db.pragma('busy_timeout = 5000');
      db.pragma('foreign_keys = ON');
      db.prepare(
        "DELETE FROM reasoning_patterns WHERE task_type = 'book-knowledge' AND tags LIKE ?",
      ).run(`%"${book}"%`);
    } finally {
      db.close();
    }
...
function jsonObj(s: string | null): Record<string, unknown> {
  if (s === null) return {};
  try {
...
function jsonPages(s: string | null): number[] | undefined {
...
function worthScore(meta: Record<string, unknown>): number {
  switch (meta['worth']) {
    case 'high':
      return 0.9;
    case 'medium':
...
function brainKindOf(v: unknown): 'book' | 'repo' | 'paper' {
...
function licenseGate(
  ...
  if (kind === 'book' || override === true) return undefined;
...
function stampBrainMeta(ku: BookKU, src: BrainSource | undefined): BookKU {
...
function stripBrainMeta(ku: BookKU): BookKU {
  if (ku.metadata === undefined) return ku;
...
export async function importBrainSlice(opts: {
  ...
  const license = typeof meta0[BRAIN_LICENSE_KEY] === 'string' ? (meta0[BRAIN_LICENSE_KEY] as string) : undefined;
  ...
  if (gate !== undefined) return { sources: [], kus: 0, error: gate };
...
export async function registerKusToBrain(opts: {
  ...
  if (gate !== undefined) return { kus: 0, error: gate };
...
export async function queryBrain(opts: {
  ...
  const run = async (match: 'all' | 'any' | undefined): Promise<{ hits: BookKUHit[]; error?: string }> => {
    ...
    if (match !== undefined) qopts.match = match;
  ...
  if (opts.match === 'any' || first.error !== undefined || first.hits.length > 0) return first;
