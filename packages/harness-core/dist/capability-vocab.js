...
export const SECRET_FILE_RE = /\.env\b|\.env\.|id_rsa|id_ed25519|id_ecdsa|\.ssh\/|\.aws\/|\.config\/gcloud|application_default_credentials|\.npmrc|\.netrc|\.git-credentials|\.kube\/config|\.pem\b|\.p12\b|\.pfx\b|\.key\b/i;
 ...
 */
export function stripCodeNoise(text) {
...
const CMD_SHELL_RE = new RegExp(`${CMD_PREFIX}(${[...PACKAGE_RUNNERS].join('|')}|python\\d?|node|nodejs|bash|sh|zsh|ruby|perl|php)\\b${ARG_FOLLOWS}`, 'i');
// network via library identifiers (matched on noise-stripped code — never in a quote).
const NETWORK_CALL_RE = /\bfetch\s*\(|\baxios\b|WebFetch\s*\(|\bWebSearch\b|\brequests\.|\burllib3?\b|\bhttpx\b|\baiohttp\b|\bparamiko\b|\bsmtplib\b|\bftplib\b|\bhttp\.client\b|\bsocket\.(?:socket|create_connection)\s*\(|\b(?:https?|net|dgram|tls|dns)\.(?:get|request|createConnection|connect|createSocket|resolve\w*)\s*\(|\bnew\s+WebSocket\b|(?:require\(|from\s+)['"](?:node:)?(?:http|https|net|dgram|tls|ws|node-fetch|got|undici)['"]/;
// shell exec via library identifiers (language-agnostic).
const EXEC_CALL_RE = /child_process|execSync|execFileSync|spawnSync|\bspawn\s*\(|\bexec(?:File)?\s*\(|\bsubprocess\b|\bos\.system\b|\bos\.popen\b|\bPopen\b|\bOpen3\b|\bcommands\.get(?:status)?output\b|%x[([{]|(?:^|[^.\w])system\s*\(/m;
function isProbablyBinary(buf) {
    return /[\x00-\x08\x0E-\x1F]/.test(buf);
}
function collectScriptFiles(dir, depth, acc) {
    if (depth > 3 || acc.length >= 200)
        return;
    let entries;
    try {
 ...
 */
export function detectScriptCapabilities(skillDir) {
    const out = { network: false, shell: false };
    ...
    for (const abs of files) {
        ...
        if (isShellExt || (ext === '' && /^#![^\n]*\b(bash|sh|zsh)\b/.test(raw)))
            out.shell = true;
        if (!SCRIPT_EXTS.has(ext) && !hasShebang)
            continue;
        ...
        if (CMD_NET_RE.test(commentless) || NETWORK_CALL_RE.test(noiseless))
            out.network = true;
        if (out.shell || CMD_SHELL_RE.test(commentless) || EXEC_CALL_RE.test(noiseless))
            out.shell = true;
    }
    return out;
}
 ...
 */
export function parseDeclaredCapabilities(skillMd) {
    const fmMatch = /^---\r?\n([\s\S]*?)\r?\n---/.exec(skillMd);
 ...
 */
export function parseDeclaredLimits(skillMd) {
    const fmMatch = /^---\r?\n([\s\S]*?)\r?\n---/.exec(skillMd);
    const fm = fmMatch ? (fmMatch[1] ?? '') : '';
    ...
    for (let i = limitsIdx + 1; i < capLines.length; i++) {
        ...
        if (indent.length <= childIndent.length)
            break; // back to a sibling → end of limits
        body.push(l);
    }
    ...
    if (tt !== undefined)
        out.toolTimeoutMs = tt;
    if (mc !== undefined)
        out.maxToolCallsPerTurn = mc;
    if (ra !== undefined)
        out.requireApprovalForDangerous = ra;
    return out;
}
//# sourceMappingURL=capability-vocab.js.map