...
export function isSafeLockName(name) {
...
export function namedLockPath(projectRoot, name) {
...
export class NamedLockNameError extends Error {
    code = 'ELOCKNAME';
    constructor(name) {
        super(`invalid lock name ${JSON.stringify(name)} — a lock name must match ${String(SAFE_LOCK_NAME)} ` +
...
export class NamedLockTimeoutError extends Error {
    ...
    constructor(lockPath, waitedMs) {
        super(`the lock at ${lockPath} stayed held for ${waitedMs}ms — another process is still inside the ` +
 ...
 */
const lockfileInternals = (() => {
    try {
        const req = createRequire(import.meta.url);
        const mod = req('proper-lockfile/lib/lockfile.js');
        return typeof mod.getLocks === 'function' ? { getLocks: mod.getLocks } : {};
    }
    catch {
        return {};
    }
...
function disarmExitRemoval(resourceKey) {
    const get = lockfileInternals.getLocks;
    if (get === undefined)
        return false;
    try {
        const locks = get();
        if (Object.prototype.hasOwnProperty.call(locks, resourceKey))
...
export function withNamedLockSync(projectRoot, name, fn, opts = {}) {
    ...
    for (;;) {
        ...
        if (got !== 'held') {
            ...
            if (outcome !== 'released') {
                throw new NamedLockCompromisedError(lockPath, compromised ??
                    new Error('the lock directory no longer carries this holder\u2019s ownership token; a waiter may have broken it ' +
        ...
        const now = Date.now();
        if (now >= deadline)
            throw new NamedLockTimeoutError(lockPath, now - started);
        sleepSync(Math.min(pollMs, deadline - now));
    }
}
//# sourceMappingURL=named-lock.js.map