...
export function buildSetupPlan(scan) {
    const missing = [];
    if (!scan.visionExists)
        missing.push(P_VISION);
    if (!scan.manifestExists)
        missing.push(P_MANIFEST);
    if (!scan.testingExists)
        missing.push(P_TESTING);
    if (!scan.projectSkillsExists)
        missing.push(P_PROJECT_SKILLS);
    const suggestions = [];
    if (!scan.manifestExists && scan.packages.length > 0)
        suggestions.push(`auto-scaffold ${P_MANIFEST} from ${scan.packages.length} discovered package(s) — you refine the grouping`);
    if (!scan.visionExists)
        suggestions.push(`author ${P_VISION} from a short interview (seeded from the README)`);
    if (!scan.testingExists)
        suggestions.push(`author ${P_TESTING} from a short interview (test commands, definition of done, gates)`);
    if (scan.reviewCorpus)
        suggestions.push('generate the `critic` role from your review history: `dz mr-rakes --gen-critic`');
    return {
...
export function suggestSubsystems(packages) {
    ...
    for (const p of pkgs) {
        if (p.startsWith('skills-'))
            arsenal.push(p);
        else if (/^(core|.*-core|.*-cli|.*-presets|adapter-)/.test(p))
            foundation.push(p);
        else
            other.push(p);
    }
    ...
    if (foundation.length)
        subs.push({ id: 'foundation', label: 'Foundation', job: null, desc: 'Core, CLI, presets, adapters — the substrate.', packages: foundation.filter((p) => !p.startsWith('adapter-')), packagePatterns: foundation.some((p) => p.startsWith('adapter-')) ? ['adapter-*'] : [], commands: [] });
    if (arsenal.length)
        subs.push({ id: 'arsenal', label: 'Arsenal', job: null, desc: 'The skills themselves.', packages: [], packagePatterns: ['skills-*'], commands: [] });
    if (other.length)
        subs.push({ id: 'app', label: 'Application', job: null, desc: 'Product-specific packages (rename/split to your real subsystems).', packages: other, packagePatterns: [], commands: [] });
    return subs;
}
/** architecture/vision.md from the spec. Deterministic. */
export function renderVisionDoc(v) {
    ...
    if (v.direction) {
        lines.push('', '## Where it is going', v.direction.trim());
    }
    if (v.boundaries && v.boundaries.length) {
        lines.push('', '## What it consciously does NOT do');
        for (const b of v.boundaries)
            lines.push(`- ${b}`);
    }
    if (v.principles && v.principles.length) {
        lines.push('', '## Principles that must not be broken');
        v.principles.forEach((p, i) => lines.push(`${i + 1}. ${p}`));
    }
    return lines.join('\n') + '\n';
}
/** architecture/testing.md from the spec — the project's verification procedure (feeds Step 8 QE). Deterministic. */
export function renderTestingDoc(t) {
    ...
    if (t.doneDefinition) {
        lines.push('## Definition of "done"', t.doneDefinition.trim(), '');
    }
...
export function renderDegradationsDoc() {
    return [
...
export function buildManifestFromSpec(subs) {
...
export function buildProjectSkillsFromSpec(spec) {
    const roles = { 'product-vision': P_VISION };
    if (spec.testing)
        roles['testing'] = P_TESTING;
    const r = spec.roles ?? {};
    if (r.critic)
        roles['critic'] = r.critic === 'auto' ? P_CRITIC : r.critic;
    if (r.brand)
        roles['brand'] = r.brand;
    if (r['impl-bar'])
        roles['impl-bar'] = r['impl-bar'];
 ...
 */
function mergeManifest(existingObj, nextSubs) {
    if (!isObj(existingObj) || !Array.isArray(existingObj.subsystems))
        return null; // malformed shape → don't touch
    const ids = new Set();
    for (const s of existingObj.subsystems)
...
function mergeProjectSkills(existingObj, nextRoles, nextExtra) {
    if (!isObj(existingObj))
        return null;
    const existingRoles = existingObj.roles;
    if (existingRoles !== undefined && !isObj(existingRoles))
        return null; // roles present but not an object → don't touch
    const roles = { ...(isObj(existingRoles) ? existingRoles : {}) };
    const added = [];
    for (const [k, v] of Object.entries(nextRoles))
        if (!(k in roles) && typeof v === 'string') {
            roles[k] = v;
            added.push(`roles.${k}`);
        }
    const existingExtra = existingObj.extra;
    if (existingExtra !== undefined && !Array.isArray(existingExtra))
        return null; // extra present but not an array → don't touch
    const extra = Array.isArray(existingExtra) ? [...existingExtra] : [];
    const seen = new Set(extra.filter(isObj).map((e) => `${String(e.skill)}|${String(e.phase)}`));
    for (const e of nextExtra) {
        const key = `${e.skill}|${e.phase}`;
        if (!seen.has(key)) {
            extra.push(e);
            seen.add(key);
            added.push(`extra:${e.skill}`);
        }
    }
    const merged = { ...existingObj, roles, ...(extra.length ? { extra } : {}) };
    return { content: JSON.stringify(merged, null, 2) + '\n', added };
}
...
function proseFile(path, existing, render) {
    if (!existing.exists)
        return { path, action: 'create', content: render() };
...
function structuredFile(path, existing, render, merge) {
    if (!existing.exists)
        return { path, action: 'create', content: render() };
    if (existing.content === undefined)
        return { path, action: 'unchanged', content: '', note: 'exists but unreadable — left as-is (never overwritten)' };
    const parsed = tryParse(existing.content);
    if (parsed === PARSE_FAIL)
        return { path, action: 'unchanged', content: existing.content, note: 'exists but not valid JSON — left as-is; fix by hand' };
    const m = merge(parsed);
    if (m === null)
        return { path, action: 'unchanged', content: existing.content, note: 'exists with a non-standard shape — left as-is; merge by hand' };
...
export function renderGuardsConfig(opts = {}) {
 ...
 */
export function renderGuardsRunner() {
    return [
        '#!/usr/bin/env node',
 ...
 */
export function renderGatesDoc(target) {
...
export function scaffoldFromSpec(spec, existing, target = 'agents-md') {
    const files = [];
    if (spec.vision)
        files.push(proseFile(P_VISION, existing.vision, () => renderVisionDoc(spec.vision)));
    if (spec.testing)
        files.push(proseFile(P_TESTING, existing.testing, () => renderTestingDoc(spec.testing)));
    ...
    files.push(structuredFile(P_PROJECT_SKILLS, existing.projectSkills, () => JSON.stringify(nextPS, null, 2) + '\n', (parsed) => mergeProjectSkills(parsed, (nextPS.roles ?? {}), nextPS.extra ?? [])));
    if (spec.degradations)
        files.push(proseFile(P_DEGRADATIONS, existing.degradations ?? { exists: false }, renderDegradationsDoc));
    if (spec.guards) {
    ...
    if (spec.gates)
        files.push(proseFile(P_GATES_DOC, existing.gatesDoc ?? { exists: false }, () => renderGatesDoc(target)));
    return { files: [...files].sort((a, b) => byStr(a.path, b.path)) };
}
/** Human preview of the scaffold plan. Deterministic. */
export function renderScaffoldPreview(result) {
    const icon = (a) => (a === 'create' ? '✚ create' : a === 'augment' ? '✎ augment' : '· unchanged');
    const lines = ['feature-adr-setup — planned files (pass --apply to write; existing files are never clobbered):', ''];
    for (const f of result.files)
        lines.push(`  ${icon(f.action)}  ${f.path}${f.note ? `  — ${f.note}` : ''}`);
    return lines.join('\n');
}
...
function readExistingFile(p) {
    ...
    if (!exists)
        return { exists: false };
    try {
        return { exists: true, content: readFileSync(p, 'utf8') };
    }
    catch {
        return { exists: true };
    }
...
export function scanForSetup(repoRoot) {
    const has = (rel) => { try {
    ...
    try {
        packages = scanWorkspacePackages(repoRoot).map((p) => p.name);
    }
    catch { /* none */ }
    try {
        reviewCorpus = findReviewArtifacts(repoRoot).length > 0;
    }
    catch { /* none */ }
    return {
...
export function readExistingForScaffold(repoRoot) {
    return {
...
//# sourceMappingURL=feature-adr-setup.js.map