...
export function stemToken(token) {
    ...
    if (CYRILLIC_TOKEN.test(normalized))
        return stemCyrillic(normalized);
    if (LATIN_TOKEN.test(normalized))
        return stemLatin(normalized);
    return normalized;
}
