...
export function adjustForContext(context) {
    const c = (context ?? '').toLowerCase();
    if (c.includes('performance'))
        return { performance: 0.6, efficiency: 0.2, stability: 0.1, capability: 0.1 };
    if (c.includes('efficiency'))
        return { performance: 0.2, efficiency: 0.6, stability: 0.1, capability: 0.1 };
    if (c.includes('stability'))
        return { performance: 0.2, efficiency: 0.1, stability: 0.6, capability: 0.1 };
    if (c.includes('capability'))
        return { performance: 0.1, efficiency: 0.1, stability: 0.2, capability: 0.6 };
 ...
 */
export function renormalizeOverActive(w, active = { capability: false }) {
    const on = (k) => active[k] !== false;
    const sum = ['performance', 'efficiency', 'stability', 'capability']
...
export function deltaEvaluate(cur, prev, weights = DEFAULT_WEIGHTS) {
    const m = {
    ...
    return (weights.performance * m.performance +
 ...
 */
export function lessonDeltaFromHistory(lesson, weights = DEFAULT_WEIGHTS) {
    const ev = [...lesson.events].filter((e) => typeof e.t === 'number' && isFinite(e.t)).sort((a, b) => a.t - b.t);
    if (ev.length < 2)
        return { id: lesson.id, delta: 0, pruneCandidate: false, hasSignal: false };
    ...
    const delta = w.performance * m.performance + w.efficiency * m.efficiency + w.stability * m.stability + w.capability * m.capability;
...
export function rankLessonsByDelta(lessons, weights = DEFAULT_WEIGHTS) {
    return lessons.map((l) => lessonDeltaFromHistory(l, weights)).sort((a, b) => b.delta - a.delta);
}
//# sourceMappingURL=safla-delta.js.map