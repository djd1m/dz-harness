...
function readBoundedJson(path, label) {
    ...
    if (st.isSymbolicLink() || !st.isFile())
        throw new IntegrationApplyError('UNSAFE_PATH', `${label} must be a regular non-symlink file`);
    if (st.size > MAX_CARRIER_BYTES)
        throw new IntegrationApplyError('CONFIG_MALFORMED', `${label} exceeds ${MAX_CARRIER_BYTES} bytes`);
    const bytes = readFileSync(path, 'utf8');
    try {
        return { bytes, value: assertPlainObject(parseStrictJson(bytes, { label, maxBytes: MAX_CARRIER_BYTES }), label) };
    }
    catch (error) {
        if (error instanceof IntegrationApplyError)
            throw error;
...
function journalFrom(value) {
    if (value['version'] !== 1)
        throw new IntegrationApplyError('CONFIG_MALFORMED', 'integration journal version must be 1');
    ...
    for (const [key, raw] of Object.entries(rawEntries)) {
        ...
        if (state !== 'pending' && state !== 'committed')
            throw new IntegrationApplyError('CONFIG_MALFORMED', `integration journal entry ${key} has invalid state`);
        for (const field of ['carrierPath', 'entryId', 'previousCarrierDigest', 'desiredCarrierDigest', 'desiredContentHash']) {
            if (typeof row[field] !== 'string')
                throw new IntegrationApplyError('CONFIG_MALFORMED', `integration journal entry ${key} lacks ${field}`);
        }
        entries[key] = {
            state,
            carrierPath: row['carrierPath'],
            entryId: row['entryId'],
            previousCarrierDigest: row['previousCarrierDigest'],
            desiredCarrierDigest: row['desiredCarrierDigest'],
            desiredContentHash: row['desiredContentHash'],
        };
    }
    return { version: 1, entries };
}
function ensureSafeProjectRoot(projectRoot) {
    ...
    if (parent !== root) {
        ...
        for (const part of relative(root, parent).split(sep).filter(Boolean)) {
            ...
            if (st.isSymbolicLink() || !st.isDirectory())
                throw new IntegrationApplyError('UNSAFE_PATH', `carrier parent is not a real directory: ${relativePath}`);
        }
    }
...
function durableReplace(path, bytes) {
    ...
    try {
        ...
        if (reread !== bytes)
            throw new IntegrationApplyError('APPLY_FAILED', `durable reread mismatch for ${path}`);
...
function isKnownAgentDbLegacy(id, value, root) {
    if (id !== 'mcpServers.agentdb' || value === null || typeof value !== 'object' || Array.isArray(value))
        return false;
    const row = value;
    if (!Object.keys(row).every((key) => ['command', 'args', 'env'].includes(key)))
        return false;
    if (row['command'] !== 'npx' || !Array.isArray(row['args']))
        return false;
    const args = row['args'];
    if (args.length !== 3 || typeof args[0] !== 'string' || !/^agentdb@(?:latest|[^\s/]+)$/.test(args[0]) || args[1] !== 'mcp' || args[2] !== 'start')
        return false;
    if (row['env'] === undefined)
        return true;
    const env = row['env'];
    if (env === null || typeof env !== 'object' || Array.isArray(env) || Object.keys(env).length !== 1)
        return false;
    const path = env['AGENTDB_PATH'];
...
export function applyIntegrationFragments(options) {
    ...
    for (const fragment of options.fragments) {
        ...
        rows.push(fragment);
        groups.set(fragment.carrierPath, rows);
    }
    if (groups.size === 0)
    ...
    return withNamedLockSync(root, 'integrations', () => {
        ...
        const journal = journalRead === undefined ? { version: 1, entries: Object.create(null) } : journalFrom(journalRead.value);
        const candidates = [];
        ...
        for (const [carrierRel, fragments] of [...groups.entries()].sort(([a], [b]) => a.localeCompare(b))) {
            ...
            const rootObject = existing?.value ?? Object.create(null);
            const candidate = { ...rootObject };
            const tuples = [];
            for (const fragment of fragments) {
                const rootValue = candidate[fragment.rootKey];
                const bucket = rootValue === undefined ? Object.create(null) : assertPlainObject(rootValue, `${carrierRel}.${fragment.rootKey}`);
            ...
            for (const tuple of tuples)
                pendingEntries[tuple] = { ...pendingEntries[tuple], desiredCarrierDigest };
        ...
        if (options.injectFault === 'after-pending')
            throw new IntegrationApplyError('APPLY_FAILED', 'injected fault after pending journal publication');
        const written = [];
        ...
        try {
            for (const candidate of candidates) {
                ...
                if (current !== candidate.before)
                    throw new IntegrationApplyError('CONCURRENT_MODIFICATION', `${candidate.rel} changed after planning`);
                if (current === candidate.bytes)
                    alreadyCurrent.push(candidate.rel);
                else {
            ...
            const committedEntries = { ...pendingEntries };
            for (const candidate of candidates) {
                for (const tuple of candidate.tuples)
                    committedEntries[tuple] = { ...committedEntries[tuple], state: 'committed' };
            }
            durableReplace(journalPath, `${JSON.stringify({ version: 1, entries: committedEntries }, null, 2)}\n`);
            if (options.injectFault === 'after-committed')
                throw new IntegrationApplyError('APPLY_FAILED', 'injected fault after committed journal publication', true);
            return { written, alreadyCurrent, journalPath: JOURNAL_REL };
        }
        catch (error) {
            if (error instanceof IntegrationApplyError && error.applied)
                throw error;
            if (carrierPublished) {
                throw new IntegrationApplyError('APPLY_FAILED', `carrier was written but committed ownership journal was not verified: ${error instanceof Error ? error.message : String(error)}`, true);
            }
            throw error;
        }
    });
}
//# sourceMappingURL=integration-apply.js.map