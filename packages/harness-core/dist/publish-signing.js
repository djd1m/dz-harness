 ...
 */
const ok = (verdict, reason) => ({ verdict, blocking: false, reason });
const stop = (verdict, reason) => ({ verdict, blocking: true, reason });
export function decidePublishSigning(input) {
    ...
    if (input.dryRun)
        return ok('skip-dry-run', 'dry run — nothing is signed and nothing is written');
    // No tarball, no recipient: refusing here would block a bump for a signature nobody will receive.
    if (input.bumpOnly === true)
        return ok('skip-bump-only', '--bump-only — no tarball is produced, so no signature can reach anyone');
    ...
    if (input.keyPath === null || !input.keyExists) {
        return stop('refuse-no-key', 'this pack is signed but no signing key is available, so publishing would ship a manifest the version bump already invalidated — pass --sign-key <path outside the repo>, or unsign the pack deliberately');
    }
...
export function publishSigningLine(pack, d) {
...
export function decidePostSigningVerification(input) {
    ...
    if (input.pack !== undefined && input.verifiedPack !== undefined && input.pack !== input.verifiedPack) {
        return stop('refuse-unverified-after-signing', `the verification ran against \`${input.verifiedPack}\` but the pack being published is \`${input.pack}\` — a pass about another artifact is not a pass`);
    }
    if (!input.trustRootPresent) {
        // Not established is not a pass: we cannot claim the signature is good, and we must not pretend.
        return stop('refuse-unverified-after-signing', 'the pack was re-signed but there is no trust root to verify the result against — publishing would ship a signature nobody here could check');
    }
    if (!input.verifiesAgainstTrustRoot) {
        return stop('refuse-unverified-after-signing', 'the pack was re-signed but does not verify against the trust root — most likely the WRONG signing key; publishing it would hand every consumer a TAMPERED verdict');
    }
 ...
 */
function assertUsableInventory(packed) {
    if (packed.length === 0) {
    ...
    for (const p of packed) {
        if (p !== p.trim() || p === '')
            throw new UnusableInventoryError(`packed path is blank or padded: ${JSON.stringify(p)}`);
        if (p.startsWith('/') || /^[A-Za-z]:/.test(p))
            throw new UnusableInventoryError(`packed path is absolute: ${p}`);
        if (p.startsWith('./') || p.includes('../') || p.includes('\\'))
            throw new UnusableInventoryError(`packed path is not a normal POSIX relative path: ${p}`);
        const key = p.toLowerCase();
        if (seen.has(key))
            throw new UnusableInventoryError(`packed inventory contains a duplicate or case-collision: ${p}`);
        seen.add(key);
    }
}
...
export function signableSetLine(pack, d) {
    const parts = [`${d.sign.length} signed`];
    if (d.droppedNotPublished.length > 0)
        parts.push(`${d.droppedNotPublished.length} on disk but never shipped (${d.droppedNotPublished.slice(0, 3).join(', ')})`);
    if (d.publishedButUnsigned.length > 0)
        parts.push(`${d.publishedButUnsigned.length} SHIPPED BUT UNSIGNED (${d.publishedButUnsigned.slice(0, 3).join(', ')})`);
    return `signable set (${pack}): ${parts.join(' · ')}`;
}
//# sourceMappingURL=publish-signing.js.map