...
import { QE_SEVERITIES, findInvalidQeVerdictLines, parseQeFindings, readQeVerdictLines } from './qe-findings.js';
...
export function readMutationEvidence(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (verdictCol < 0 || !header.some((c) => /mutation|mutant/i.test(c)))
            continue;
        let rows = 0;
        let proven = 0;
        let recognised = 0;
        for (let j = i + 1; j < lines.length; j++) {
            const row = lines[j];
            if (row === undefined || !row.trim().startsWith('|'))
                break;
            ...
            if (!MUTATION_VERDICTS.has(token))
                continue;
            recognised++;
            if (token === 'PROVEN')
                proven++;
...
export function readQeGrade(qeText) {
    ...
    if (invalidFirst.length > 0) {
        const reasons = invalidFirst.map((l) => `line ${l.line}: ${JSON.stringify(l.text.trim())} does not match "QE-VERDICT: <A|B|C|D><+|-> "`);
        return { status: 'invalid', grade: null, found: [...verdictLines, ...reasons], source: 'verdict-line' };
    }
    if (verdictLines.length === 1) {
        return { status: 'unique', grade: verdictLines[0], found: verdictLines, source: 'verdict-line' };
    }
    if (verdictLines.length > 1) {
        const distinct = [];
        for (const g of verdictLines)
            if (!distinct.includes(g))
                distinct.push(g);
    ...
    if (found.length === 0)
        return { status: 'none', grade: null, found: [], source: 'none' };
    if (found.length === 1)
        return { status: 'unique', grade: found[0], found, source: 'prose' };
...
export function scoreRun(slug, artifacts) {
    ...
    add('discrimination', 'the property test is proven able to fail', discrPasses ? 'pass' : 'absent', mutationEvidence.status === 'proven'
    ...
    const findings = findingsParsed.status === 'absent'
        ? { status: 'absent' }
...
export function renderFindingsLine(findings) {
    if (findings === undefined || findings.status === 'absent')
        return null;
    ...
    const refusedSuffix = (() => {
        if (findings.refused.length === 0)
            return '';
        const first = findings.refused[0];
...
function isDisciplineVerdict(value) {
    return value === 'pass' || value === 'partial' || value === 'absent';
}
function scoreAggregateRowFrom(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return null;
    const record = value;
    const disciplinesValue = record['disciplines'];
    if (!Array.isArray(disciplinesValue))
        return null;
    const disciplines = [];
    ...
    for (const value of disciplinesValue) {
        ...
        if (typeof discipline['id'] !== 'string' || discipline['id'].trim() === '')
            return null;
        if (!isDisciplineVerdict(discipline['verdict']))
            return null;
    ...
    if (typeof record['slug'] !== 'string' || record['slug'].trim() === '')
        return null;
    if (typeof record['qeHash'] !== 'string' || record['qeHash'].trim() === '')
        return null;
    if (typeof passed !== 'number' || !Number.isSafeInteger(passed) || passed < 0)
        return null;
    if (typeof total !== 'number' || !Number.isSafeInteger(total) || total < 0)
        return null;
    if (qeGrade !== null && (typeof qeGrade !== 'string' || qeGrade.trim() === ''))
        return null;
    if (total !== disciplines.length)
        return null;
    if (passed !== disciplines.filter((discipline) => discipline.verdict === 'pass').length)
        return null;
    return {
        ts: record['ts'],
        slug: record['slug'],
 ...
 */
export function scoreReceiptToAggregateRow(input) {
    let parsed;
    try {
    ...
    if (row === null)
        throw new Error('score receipt has an invalid RunScorecard shape');
...
export function readScoreAggregateRows(text) {
    const rows = [];
    for (const line of String(text).split('\n')) {
        if (line.trim() === '')
            continue;
        try {
            const row = scoreAggregateRowFrom(JSON.parse(line));
            if (row !== null)
                rows.push(row);
        }
        catch {
...
export function dedupeScoreAggregateRows(candidates, existing) {
...
export function buildScoreAggregateReport(rows, unreadableReceipts, appended) {
...
export function renderScoreAggregateReport(report) {
    const out = [
    ...
    for (const grade of report.grades)
        out.push(`  ${grade.grade ?? '(no grade)'}: ${grade.count}`);
    return out.join('\n');
}
