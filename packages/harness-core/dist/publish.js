...
function publishedVersion(name, exec = execSync) {
    try {
...
function maskFences(lines) {
    ...
    for (const line of lines) {
        ...
        if (fence === null && open !== null && open[1] !== undefined) {
            fence = open[1][0];
            out.push('');
            continue;
        }
        if (fence !== null) {
            out.push('');
            if (new RegExp('^ {0,3}' + fence + '{3,}\\s*$').test(line))
                fence = null;
            continue;
        }
        out.push(line);
...
export function changelogRegion(lines) {
    ...
    for (let i = start; i < masked.length; i++) {
        if (i > start && REGION_END.test(masked[i]))
            break;
        out.add(i);
    }
    return out;
