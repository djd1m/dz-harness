...
function containsStemSequence(textStems, keywordStems) {
    if (keywordStems.length === 0 || keywordStems.length > textStems.length)
        return false;
    return textStems.some((_, start) => keywordStems.every((keywordStem, offset) => textStems[start + offset] === keywordStem));
}
function keywordMatches(lowerText, textStems, keyword) {
...
function scoreSkill(entry, topics, task) {
    ...
    for (const topic of topics) {
        ...
        for (const kw of keywords) {
            if (keywordMatches(lower, textStems, kw)) {
                score += 10;
                break;
            }
        }
    ...
    if (task !== undefined && task !== '') {
        ...
        for (const w of words) {
            ...
            if (id.includes(w) || (wordStem !== undefined && idStems.has(wordStem)))
                overlap += 3;
            else if (desc.includes(w) || (wordStem !== undefined && descStems.has(wordStem)))
                overlap += 1;
        }
...
export function recommend(task, registry, projectRoot) {
    let topics = extractTopics(task);
    let topicSource = topics.length > 0 ? 'task' : 'none';
    ...
    if (topics.length === 0 && projectRoot) {
        ...
        const pretrainTopics = [];
        const techNames = analysis.techs.map((t) => t.name.toLowerCase());
        if (techNames.some((t) => t.includes('node') || t.includes('typescript')))
            pretrainTopics.push('api', 'testing');
        if (techNames.some((t) => t.includes('python') || t.includes('django') || t.includes('fastapi')))
            pretrainTopics.push('api', 'testing');
        if (techNames.some((t) => t.includes('react') || t.includes('vue') || t.includes('angular')))
            pretrainTopics.push('frontend');
        if (analysis.hasDocker)
            pretrainTopics.push('docker');
        if (analysis.hasTerraform)
            pretrainTopics.push('terraform');
        if (analysis.hasKubernetes)
            pretrainTopics.push('kubernetes');
        if (analysis.hasCI)
            pretrainTopics.push('ci-cd');
        if (analysis.hasTests)
            pretrainTopics.push('testing');
    ...
    const boostFor = (entry) => {
        const base = computePatternBoost(entry.id, entry.description, patterns);
        if (base <= 0 || backend === undefined)
            return base;
        const matched = records.filter((r) => computePatternBoost(entry.id, entry.description, [recordToPattern(r)]) > 0);
        if (matched.length === 0)
            return base;
        const signals = backend.enhance(matched.map((r) => ({ dzId: r.id, score: r.score, reinforcement: readReinforcementState(r) })), { kind: 'recommend' });
    ...
    const skills = scored.slice(0, 10).map((s) => {
    ...
    const presets = presetScores.slice(0, 3);
    // Recommend commands based on topics
    const commands = COMMAND_KB
