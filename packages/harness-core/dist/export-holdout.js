 ...
 */
export const DEFAULT_HELD_OUT_DOMAINS = ['health-research'];
/**
 ...
 */
export function applyExportHoldout(records, heldOut = DEFAULT_HELD_OUT_DOMAINS) {
    const targets = new Set(heldOut.map(canonicalDomainKey).filter((d) => d !== ''));
    if (targets.size === 0)
        return { exported: records, withheld: [], domains: [] };
    const exported = [];
    const withheld = [];
 ...
 */
export function renderHoldoutNote(result) {
...
export function heldOutAfterOptIn(optIn, heldOut = DEFAULT_HELD_OUT_DOMAINS) {
    if (optIn === undefined)
        return heldOut;
    const requested = new Set(optIn.split(',').map((d) => canonicalDomainKey(d)).filter((d) => d !== ''));
 ...
 */
export function renderSharedStoreAdvice(domain, resolvedProjectRoot) {
    const key = canonicalDomainKey(domain);
    if (key === '' || !DEFAULT_HELD_OUT_DOMAINS.map(canonicalDomainKey).includes(key))
        return '';
    if (/[\\/]\.health-brain\/?$/.test(resolvedProjectRoot))
        return '';
    return [
...
export function decideVectorExport(input) {
    ...
    const optedKeys = new Set((input.optedIn ?? '').split(',').map((d) => canonicalDomainKey(d)).filter((d) => d !== ''));
...
//# sourceMappingURL=export-holdout.js.map