 ...
 */
import { buildReqeVerdict } from './reqe-verdict.js';
export const REQE_SCHEMA = 'reqe-due-1';
export const REQE_SCOPE = 'scope: same-family Step-8 QE records cause=usage-switched, probe-failed, same-family-fallback ' +
...
export function parseReqeDebt(text) {
    ...
    if (d.cause !== undefined && d.cause !== 'usage-switched' && d.cause !== 'probe-failed' &&
        d.cause !== 'same-family-fallback' && d.cause !== 'same-family-pinned')
        return null;
...
export function countReqeByCause(debts) {
    const counts = {
