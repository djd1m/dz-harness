 ...
 */
export async function putBookKnowledge(projectRoot, kus, opts = {}) {
    if (kus.length === 0)
        return { upserted: 0, evicted: 0 };
    ...
    if ('error' in opened)
        return { upserted: 0, evicted: 0, error: opened.error };
    const db = opened;
    try {
        ...
        const delOrphan = db.prepare(`DELETE FROM book_knowledge WHERE book = ? AND ku_id NOT IN (${kuIds.map(() => '?').join(', ')})`);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        const run = db.transaction(() => {
            const evictedStale = del.run(book, corpusVersion).changes ?? 0;
            const evictedOrphan = delOrphan.run(book, ...kuIds).changes ?? 0;
            for (const ku of kus) {
                delKu.run(ku.book, ku.kuId);
                ins.run(ku.book, ku.kuId, ku.corpusVersion, ku.type, ku.name, ku.problem, ku.content, ku.chapter ?? null, ku.pages ? JSON.stringify(ku.pages) : null, ku.metadata ? JSON.stringify(ku.metadata) : null);
            }
...
export async function queryBookKnowledge(projectRoot, query, opts = {}) {
    ...
    try {
        ...
        return {
            hits: rows.map((r) => ({
                ...
                ...(r.pages !== null ? { pages: JSON.parse(r.pages) } : {}),
            })),
        };
    }
    catch (err) {
...
//# sourceMappingURL=book-kb.js.map