...
export function decideCaptureMode(opts) {
    if (!opts.enabled)
...
export function captureFailureRecord(stage, mode, reason, detail) {
...
export function serializeTrainingPair(pair) {
    try {
...
export function trainingPairAppendCmd(repoAbs, slug, stage, line) {
...
export function trainingPairBackfillCmd(repoAbs, slug, stage, lines, markKey) {
 ...
 * module cycle — H6.
 */
export function codeCheckpointPersistAllowed(landingStatus, barrierRequired) {
    if (landingStatus === 'landed')
        return barrierRequired === true;
    if (landingStatus === 'synchronous')
        return barrierRequired === false;
    return false;
}
/**
 ...
 */
export function codeStageResultShapeValid(r) {
    if (!r || typeof r !== 'object')
        return false;
    const v = r;
    if (!v.code || typeof v.code !== 'object')
        return false;
    if (typeof v.coderUsed !== 'string')
        return false;
...
export function decideCheckpointWrite(opts) {
    ...
    if (stage === '')
        return { ok: false, reason: 'stage is empty' };
    if (String(opts.inputHash ?? '').trim() === '')
        return { ok: false, reason: 'inputHash is empty' };
