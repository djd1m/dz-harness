...
export const INCOMPLETE_REASON_CODES = ['sandbox-metrics-unavailable', 'manual-entry'];
...
export function decideRecordWrite(input) {
    ...
    if (kind === 'ledger' && stamped['auto'] === true) {
        ...
        const existingReasons = Array.isArray(existingReasonsRaw)
            ? existingReasonsRaw.filter((r) => typeof r === 'string')
            : [];
        ...
        if (suppliedReason !== null && !INCOMPLETE_REASON_CODES.includes(suppliedReason)) {
        ...
        if (!complete) {
            ...
            const reasonKnown = reason !== null && INCOMPLETE_REASON_CODES.includes(reason);
        ...
        if (!complete)
            stamped['incompleteReasons'] = mergedReasons.length > 0 ? mergedReasons : existingReasons;
    }
    let line;
    try {
 ...
 */
export function applyTaskId(row, lookup) {
