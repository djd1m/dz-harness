export const VOLUME_SHADOW_SCHEMA_VERSION = 'volume-shadow/v1';
...
const EMPTY_RESULT = { observations: [], signals: [], notes: [] };
...
const RULE_ORDER = new Map(VOLUME_SHADOW_RULE_IDS.map((id, index) => [id, index]));
...
function validByteCount(value, allowZero = false) {
    return typeof value === 'number'
        && Number.isSafeInteger(value)
        && (allowZero ? value >= 0 : value > 0);
}
export function estimateTemplateTokens(bytes, cyrillicUtf8Bytes) {
...
function unknownObservation(rule, metric, scope, unit, reason, detail) {
...
function unknownTemplateTarget(scope, reason, detail) {
    return [
        unknownObservation('template-context-token-weight', 'replicated-context-estimated-tokens', scope, 'estimated_tokens', reason, detail),
        unknownObservation('template-context-largest-file-share', 'largest-context-file-share', scope, 'fraction_of_corpus', reason, detail),
    ];
}
function unknownFeature(scope, reason, detail) {
    return [
        unknownObservation('feature-artifact-diff-ratio', 'feature-artifact-diff-ratio', scope, 'byte_ratio', reason, detail),
        unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', reason, detail),
    ];
}
export function unknownVolumeShadow(reason, detail) {
    const observations = [
...
function classifyTemplateFact(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return { status: 'unknown', reason: 'malformed-file-fact' };
    const fact = raw;
    if (typeof fact.kind === 'string' && VOLUME_SCOPE.excluded.includes(fact.kind)) {
        return { status: 'excluded' };
    }
    const path = safeRelativePath(fact.path);
    if (path === undefined)
        return { status: 'unknown', reason: 'unsafe-file-path' };
...
function evaluateTemplateTarget(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    ...
    if (!Array.isArray(target.files))
        return unknownTemplateTarget(scope, 'malformed-template-files');
    const rows = [];
    const paths = new Set();
    for (const candidate of target.files) {
        ...
        if (row.status === 'unknown')
            return unknownTemplateTarget(scope, row.reason);
        if (paths.has(row.path))
            return unknownTemplateTarget(scope, 'duplicate-template-path', row.path);
        paths.add(row.path);
        rows.push(row);
    }
    rows.sort((a, b) => a.path.localeCompare(b.path));
    if (rows.length === 0)
        return unknownTemplateTarget(scope, 'empty-template-corpus');
    let bytes = 0;
    let total = 0;
    let rules = 0;
    let commands = 0;
    ...
    for (const row of rows) {
        ...
        if (row.kind === 'rules')
            rules += row.estimate.estimatedTokens;
        if (row.kind === 'commands')
            commands += row.estimate.estimatedTokens;
        if (row.kind === 'skills')
            skills += row.estimate.estimatedTokens;
        if (row.estimate.estimatedTokens > largest.estimate.estimatedTokens)
            largest = row;
    }
    ...
    const totalStatus = hasReference
        ? totalSignal ? 'outside-reference' : 'within-reference'
        : 'measured';
    const largestStatus = hasReference
...
function normalizedStep(value) {
    if (typeof value === 'number' && Number.isFinite(value))
        return String(value);
    if (typeof value === 'string' && /^(?:[0-9]|10)(?:\.5)?$/.test(value.trim()))
        return value.trim();
    return undefined;
}
export function expectedFeatureArtifacts(tier, activeSteps) {
    ...
    for (const raw of activeSteps) {
        ...
        if (step === undefined)
            continue;
        for (const artifact of STEP_ARTIFACT.get(step) ?? [])
            expected.add(artifact);
    }
    return [...expected].sort();
}
function isArtifactContentPath(path) {
    return /^(?:0[0-9](?:[_.-][^/]*)?\.md)$/.test(path)
        || /^03_adr\/.+$/.test(path)
        || /^07_code_changes\/.+$/.test(path);
}
function isExpectedPresent(expected, present) {
    if (expected.endsWith('/'))
        return [...present].some((path) => path.startsWith(expected));
    return present.has(expected);
}
function dueArtifacts(expected, lifecycle) {
    if (lifecycle === undefined || lifecycle === null || typeof lifecycle !== 'object')
        return undefined;
    if (lifecycle.phase === 'complete')
        return expected;
    const through = lifecycle.completedThroughStep;
    if (typeof through !== 'number' || !Number.isFinite(through) || through < 0 || through > 10)
        return undefined;
...
function evaluateFeature(raw) {
    ...
    if (!Array.isArray(feature.artifacts))
        return unknownFeature(scope, 'malformed-artifact-files');
    let artifactBytes = 0;
    ...
    for (const rawArtifact of feature.artifacts) {
        ...
        if (path === undefined)
            return unknownFeature(scope, 'unsafe-artifact-path');
        if (present.has(path))
            return unknownFeature(scope, 'duplicate-artifact-path', path);
        present.add(path);
        if (!isArtifactContentPath(path))
            continue;
        if (!validByteCount(artifact.bytes))
            return unknownFeature(scope, 'invalid-artifact-byte-count', path);
...
function evaluateArtifactRatio(scope, artifactBytes, diff) {
    if (!validByteCount(artifactBytes)) {
        return unknownObservation('feature-artifact-diff-ratio', 'feature-artifact-diff-ratio', scope, 'byte_ratio', 'empty-artifact-scope');
    }
    if (!diff || typeof diff !== 'object' || diff.attributable !== true) {
        return unknownObservation('feature-artifact-diff-ratio', 'feature-artifact-diff-ratio', scope, 'byte_ratio', boundedDetail(diff?.reason ?? 'unattributable-diff'));
    }
    if (!validByteCount(diff.bytes)) {
        return unknownObservation('feature-artifact-diff-ratio', 'feature-artifact-diff-ratio', scope, 'byte_ratio', diff.bytes === 0 ? 'zero-diff-denominator' : 'invalid-diff-denominator');
    }
...
function evaluateArtifactSet(feature, scope, present) {
    if (!validTier(feature.tier)) {
        return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'tier-unavailable');
    }
    if (!Array.isArray(feature.activeSteps)) {
        return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'active-steps-unavailable');
    }
    if (feature.activeSteps.some((step) => normalizedStep(step) === undefined)) {
        return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'active-steps-malformed');
    }
    ...
    if (feature.namedConsumers !== undefined) {
        if (!Array.isArray(feature.namedConsumers)) {
            return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'consumer-artifacts-malformed');
        }
        for (const raw of feature.namedConsumers) {
            ...
            if (artifact === undefined || !ARTIFACT_STEP.has(artifact)) {
                return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'consumer-artifact-outside-contract');
            }
            expected.add(artifact);
        }
    }
    ...
    if (due === undefined) {
        return unknownObservation('feature-tier-artifact-set', 'feature-tier-artifacts', scope, 'artifact_set', 'lifecycle-unavailable');
    }
...
function finalize(observations) {
    observations.sort((a, b) => (RULE_ORDER.get(a.rule) ?? 99) - (RULE_ORDER.get(b.rule) ?? 99)
...
export function evaluateVolumeShadow(input) {
    if (input === undefined)
        return EMPTY_RESULT;
    ...
    for (const target of raw.templates ?? []) {
        const name = target && typeof target === 'object' && typeof target.target === 'string'
            ? target.target
            : 'unknown-target';
    ...
    for (const feature of raw.features ?? []) {
        const slug = feature && typeof feature === 'object' && typeof feature.slug === 'string'
            ? feature.slug
            : 'unknown-feature';
...
//# sourceMappingURL=guard-volume.js.map