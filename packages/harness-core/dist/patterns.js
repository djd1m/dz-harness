 ...
 */
export function resolveReinforceTarget(records, dzIdOrText) {
    ...
    if (byId !== undefined)
        return { rec: byId, matchedBy: 'id' };
    const byText = records.find((r) => r.text === dzIdOrText);
    if (byText !== undefined)
        return { rec: byText, matchedBy: 'text' };
    ...
    const only = bare[0];
    if (only !== undefined)
        return { rec: only, matchedBy: 'suffix' };
