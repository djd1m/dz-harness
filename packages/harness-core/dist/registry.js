 ...
 */
export function skillPackBaseDirs(cwd) {
    const bases = [];
    const add = (dir) => {
    ...
    for (const scope of scopes)
        add(join(cwd, 'packages', scope));
    for (const scope of scopes)
        add(join(cwd, 'node_modules', scope));
...
function readSkillDiscoveryConfig(cwd) {
    try {
        const cfg = JSON.parse(readFileSync(join(cwd, '.dz', 'config.json'), 'utf-8'));
...
export function discoverSkillPackDirs(cwd) {
    ...
    for (const base of skillPackBaseDirs(cwd)) {
        for (const e of readdirSync(base, { withFileTypes: true })) {
            ...
            const isPackDir = e.isDirectory() ||
                (e.isSymbolicLink() && (() => { try {
                    return statSync(join(base, e.name)).isDirectory();
                }
                catch {
                    return false;
                } })());
...
export function discoverVerifiablePackDirs(cwd) {
    ...
    try {
        for (const p of discoverSkillPackDirs(cwd))
            add(p.pack, p.dir);
    }
    catch {
...
function extractDescription(content) {
    ...
    if (/^[|>][+-]?\d?$/.test(inline)) {
        ...
        for (let i = idx + 1; i < lines.length; i += 1) {
            ...
            if (indent <= keyIndent)
                break; // dedent → next key or end of frontmatter
            body.push(line.trim());
        }
...
function categoryFromPack(pack) {
    ...
    if (pack.includes('qe'))
        return 'qe';
    if (pack.includes('academic'))
        return 'academic';
    if (pack.includes('ecc'))
        return 'ecc';
    if (pack.includes('meta'))
        return 'meta';
    if (pack.includes('reasoning'))
        return 'reasoning';
    if (pack.includes('news'))
        return 'news';
    ...
    if (pack.includes('taste') || pack.includes('cloner'))
        return 'design';
    if (pack.includes('pm') ||
