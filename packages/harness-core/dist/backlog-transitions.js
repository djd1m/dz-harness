...
export function transitionLogPath(projectRoot) {
 ...
 */
export function appendTransition(projectRoot, row) {
    try {
 ...
 */
export function readTransitions(projectRoot) {
    const p = transitionLogPath(projectRoot);
    if (!existsSync(p))
        return [];
    const out = [];
    for (const line of readFileSync(p, 'utf8').split('\n')) {
        ...
        try {
            ...
            if (typeof r?.id === 'string' && typeof r?.to === 'string' && typeof r?.ts === 'string')
                out.push(r);
        }
...
//# sourceMappingURL=backlog-transitions.js.map