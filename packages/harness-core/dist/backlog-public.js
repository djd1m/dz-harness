...
export const PUBLIC_SCHEMA = 'backlog-public/1';
 ...
 */
export function buildPublicSnapshot(records, snapshotDate) {
    ...
    const buckets = {};
    for (const [name, lo, hi] of TTT_BUCKETS)
        buckets[name] = tttDays.filter((d) => d >= lo && d < hi).length;
...
export function assertPublicSafe(bytes) {
    ...
    const walk = (node, at, depth) => {
        if (depth > 6)
            return { ok: false, code: 'SCHEMA_DRIFT', reason: `слишком глубокая вложенность в ${at}` };
    ...
    let groups = 0;
    for (const field of ['statusCounts', 'effortCounts', 'categoryCounts']) {
        const v = obj[field];
    ...
    if (total !== null) {
        for (const field of ['statusCounts', 'effortCounts']) {
            const v = obj[field];
            if (!v)
                continue;
 ...
 */
export function applyDeltaRule(previous, current, heldDays) {
    if (previous === null)
        return { action: 'publish', value: current };
...
//# sourceMappingURL=backlog-public.js.map