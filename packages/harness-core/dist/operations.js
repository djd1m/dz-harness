...
export async function runDoctor(options) {
    ...
    try {
        ...
        checks.push({
            name: 'backlog backup',
            ok: true,
            ...(verdict.state === 'fresh' ? {} : { level: verdict.state === 'stale' ? 'warn' : 'unknown' }),
    ...
    if (isMonorepo) {
        try {
            const parsed = JSON.parse(readFileSync(join(root, 'packages', '@dzhechkov', 'harness-cli', 'package.json'), 'utf8'));
    ...
    for (const dir of ['.agentic-qe', '.dz', '.dz/memory']) {
        ...
        try {
            ...
            continue;
        } // каталога нет — проверять нечего, и это не находка
        for (const name of names) {
            ...
            try {
                ...
                continue;
            } // нечитаемый или исчезнувший файл — не находка: мы про него ничего не установили
            if (size > RVF_HEADER_ONLY_MAX_BYTES)
                continue;
    ...
    try {
        let configuredMemoryBackend = 'unknown';
        try {
            const cfg = JSON.parse(readFileSync(join(root, '.dz', 'config.json'), 'utf-8'));
            if (cfg.memory?.backend === 'agentdb')
                configuredMemoryBackend = 'agentdb';
            else if (cfg.memory?.backend === 'jsonl')
                configuredMemoryBackend = 'jsonl';
        }
        ...
        if (configuredMemoryBackend !== 'unknown') {
            ...
            // echo must not count.
            const invokesWriter = (cmd) => cmd.split(/\s+/).some((tok) => /^(?:\.\/)?(?:.*\/)?\.dz\/agentdb-writer\.mjs$/.test(tok.replace(/^["']|["']$/g, '')));
            const WRITER_EVENTS = ['SessionStart', 'SessionEnd', 'PreCompact'];
            let eventsWithWriter = [];
        ...
        try {
            ...
            if (existsSync(settingsPath)) {
                ...
                if (applyLegWired) {
                    ...
                    const tmpdirNote = resolved.reason === 'tmpdir-short'
 ...
 */
export function probeHookLiveness(command, payload, opts = {}) {
    ...
    try {
        ...
        return { status: null, stdout: '', stderr: String(err?.message ?? err), groupKillAttempted };
    }
}
