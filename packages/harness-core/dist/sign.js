...
function buildLegacySbomV1(manifest) {
 ...
 */
const CANONICALISED_PACK_FILES = new Set(['package.json']);
...
function stablePackageJsonV2(value, preserveOrder = false, atRoot = true) {
    if (Array.isArray(value))
        return '[' + value.map((item) => stablePackageJsonV2(item, preserveOrder, false)).join(',') + ']';
    if (value !== null && typeof value === 'object') {
        const record = value;
 ...
 */
function normalizedJsonNumber(literal) {
    const match = /^(-?)(\d+)(?:\.(\d+))?(?:[eE]([+-]?\d+))?$/.exec(literal);
    if (!match || !Number.isFinite(Number(literal)))
        return null;
...
function canonicalisationIsFaithful(text) {
    const scopes = []; // Set = inside an object, null = inside an array
    let i = 0;
    while (i < text.length) {
        ...
        if (c === '"') {
            ...
            while (j < text.length && text.charAt(j) !== '"')
                j += text.charAt(j) === '\\' ? 2 : 1;
            if (j >= text.length)
                return false; // unterminated — JSON.parse will reject it anyway
            let key;
            try {
        ...
        if (c === '-' || (c >= '0' && c <= '9')) {
            const start = i;
            while (i < text.length && /[-+0-9eE.]/.test(text.charAt(i)))
                i++;
            ...
            if (exact === null || parsed === null || exact !== parsed)
                return false;
            continue;
        }
        i++;
 ...
 */
export function hashPackBytes(relPath, bytes, manifestVersion = MANIFEST_VERSION) {
    const rel = relPath.split(sep).join('/');
    if (CANONICALISED_PACK_FILES.has(rel)) {
        try {
            const text = bytes.toString('utf-8');
            if (!canonicalisationIsFaithful(text))
            ...
            throw new Error(`refusing non-canonicalisable ${rel}: ${String(error?.message ?? error)}`);
        }
    }
 ...
 */
export function hashFile(absPath, relPath, manifestVersion = MANIFEST_VERSION) {
 ...
 */
export function hashRegularFileNoFollow(absPath, relPath, manifestVersion = MANIFEST_VERSION) {
    let fd;
    try {
...
export function readRegularFileNoFollow(absPath, expectedLength) {
    let fd;
    try {
        fd = openSync(absPath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
        const stat = fstatSync(fd);
        const allowedLengths = Array.isArray(expectedLength) ? expectedLength : expectedLength === undefined ? null : [expectedLength];
        if (!stat.isFile() || (allowedLengths !== null && !allowedLengths.includes(stat.size)))
            return null;
        const bytes = readFileSync(fd);
        if (allowedLengths !== null && !allowedLengths.includes(bytes.length))
            return null;
        return bytes;
    }
    catch {
...
function isExcludedStateEntry(entry) {
    if (!EXCLUDED_STATE_NAMES.has(entry.name))
        return false;
...
function isRootGeneratedMetadata(relDir, name, isFile) {
...
export function listPackFiles(root) {
    ...
    const walk = (dir, rel) => {
        for (const e of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (isExcludedStateEntry(e))
                continue;
            if (isRootGeneratedMetadata(rel, e.name, e.isFile()))
                continue;
 ...
 */
export const OUTSIDE_FILES_REASON = 'present in the directory but not signed — outside package.json.files, the packer would not ship it';
/**
 ...
 */
export function packAllowlistFromPackageJson(pkg) {
    if (!pkg || typeof pkg !== 'object')
        return null;
    const { files, main, bin } = pkg;
    if (!Array.isArray(files) || files.length === 0 || !files.every((f) => typeof f === 'string'))
        return null;
    const entries = files.map(normalisePackPath).filter((e) => e.length > 0);
    if (entries.length === 0)
        return null;
    const binValues = typeof bin === 'string' ? [bin]
 ...
 */
function globStarMatch(pattern, rel) {
 ...
 */
export function isInsidePackAllowlist(rel, allow) {
    const p = normalisePackPath(rel);
    if (!p.includes('/') && NPM_ALWAYS_INCLUDED.test(p))
        return true;
    if (allow.main === p || allow.bins.includes(p))
        return true;
    for (const entry of allow.entries) {
        if (GLOB_CHARS.test(entry)) {
            if (globStarMatch(entry, p))
                return true;
            continue;
        }
...
function readPackAllowlist(root) {
    try {
...
export function verifyManifest(root, signed, pubKeyPem) {
    ...
    if (sbomFailure !== null)
        failures.push({ path: SBOM_NAME, reason: sbomFailure });
    ...
    const outside = [];
    const reported = allow === null ? failures : failures.map((f) => {
        if (f.reason !== 'present in the pack but not signed' || isInsidePackAllowlist(f.path, allow))
            return f;
        outside.push(f.path);
...
function buildSbomEnvelope(manifest, legacyV1PackageHash) {
    return {
        bomFormat: 'CycloneDX',
        specVersion: '1.5',
        version: 1,
        metadata: { component: { type: 'library', name: manifest.pack } },
        components: [...manifest.files].sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0)).map((f) => !legacyV1PackageHash && CANONICALISED_PACK_FILES.has(f.path)
            ? {
                type: 'file',
                name: f.path,
                properties: [
 ...
 */
export function verifySbomAgainstManifest(root, manifest) {
    if (![1, 2, MANIFEST_VERSION].includes(manifest.version)) {
    ...
    if (bytes === null)
        return 'missing, symlinked, not a regular file, or has a non-canonical byte length';
    const actual = bytes.toString('utf8');
    if (!canonicalisationIsFaithful(actual))
        return 'JSON is malformed, ambiguous, or precision-losing';
    try {
        JSON.parse(actual);
    }
    catch {
 ...
 */
export function explainPackVerificationFailure(entries) {
