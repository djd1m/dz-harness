...
import { applyLearningSignals, applyLearningSignalsWithDelta, applyLearningSignalsWithTerms, resolveLearningBackend } from './learning-backend.js';
...
import { contextKeyFor, narrowBanditReport, payoffTermsFor, recordExposures, resolveBanditConfig, } from './lesson-payoff.js';
/**
 ...
 */
export function mirrorWriterReason(projectRoot) {
    const path = join(projectRoot, '.dz', 'config.json');
    if (!existsSync(path))
        return { enabled: false, state: 'no-config' };
    let cfg;
    try {
    ...
    if (engine === 'off')
        return { enabled: false, state: 'engine-off' };
...
export function mirrorWriterExplanation(state) {
    switch (state) {
...
export function resolveVectorEngine(projectRoot) {
    ...
    let autoAgentdbFailure;
    const probeIfInstalled = (id) => isPackageInstalled(projectRoot, id)
        ? id === 'better-sqlite3'
    ...
    if (mode === 'agentdb' || mode === 'auto') {
        ...
        if (sqlite?.state === 'unusable')
            return { reason: describeNativeDep(sqlite) };
...
export function evidenceRank(backend) {
...
function compareEvidenceAsc(a, b) {
    if (Number.isNaN(a) || Number.isNaN(b))
        return Number.isNaN(a) && Number.isNaN(b) ? 0 : Number.isNaN(a) ? 1 : -1;
    return a < b ? -1 : a > b ? 1 : 0;
}
export function compareHybridHits(a, b) {
 ...
 */
export function orderHitsForReRank(hits, idOf) {
    return [...hits].sort((a, b) => compareHybridHits({ score: a.score, evidence: evidenceRank(a.backend), dzId: idOf(a.pattern) }, { score: b.score, evidence: evidenceRank(b.backend), dzId: idOf(b.pattern) }));
}
/**
...
export function mergeHybridHits(lexical, semantic, opts) {
    ...
    const evidenceOfAcc = (v) => evidenceRank(v.lex !== undefined && v.sem ? 'both' : v.lex ?? 'vector');
    const ordered = [...acc.entries()]
        .sort((a, b) => compareHybridHits({ score: a[1].score, evidence: evidenceOfAcc(a[1]), dzId: a[0] }, { score: b[1].score, evidence: evidenceOfAcc(b[1]), dzId: b[0] }));
    const toHit = ([, v]) => ({
        pattern: v.pattern,
 ...
 */
function markRecallHits(projectRoot, backend, hits, idOf, bandit) {
    const cfg = readMemoryLearningConfig(projectRoot);
    if (cfg.backend === 'off' || cfg.onRecallHits === false)
        return;
    const ts = new Date().toISOString();
    for (const h of hits)
        backend.addSample({ dzId: idOf(h.pattern), kind: 'recall-hit', reward: h.pattern.reward, ts });
    if (bandit !== undefined && bandit.deferred === true)
        return; // the caller will commit post-cut
    if (bandit !== undefined) {
        ...
        try {
            recordExposures(projectRoot, hits.map((h) => ({ dzId: idOf(h.pattern), contextKey: bandit.contextKey, ts })), bandit.explored, { moved: bandit.moved, arms: bandit.arms });
        }
...
export async function recallHybrid(projectRoot, query, opts = {}) {
    ...
    const idOf = (p) => identityToId.get(patternIdentityOf(p)) ?? patternRecordId(p);
    ...
    const dampQuarantined = (hits) => {
        if (!memCfg.quarantine)
            return [...hits];
        return hits
            .map((h) => {
            const rec = idToRecord.get(idOf(h.pattern));
            const q = rec !== undefined && readQuarantineState(rec).quarantined;
            return q ? { ...h, score: h.score * memCfg.quarantineDamp, quarantined: true } : h;
        })
            ...
            .sort((a, b) => compareHybridHits({ score: a.score, evidence: evidenceRank(a.backend), dzId: idOf(a.pattern) }, { score: b.score, evidence: evidenceRank(b.backend), dzId: idOf(b.pattern) }));
    };
    ...
    const enhance = (hits) => {
        ...
        if (banditCfg.enabled) {
            ...
            for (const c of candidates) {
                ...
                if (rec !== undefined && readQuarantineState(rec).quarantined)
                    quarantined.add(c.dzId);
            }
            ...
            const baseTerms = deltaMap === undefined
                ? []
    ...
    const banditEmission = () => banditReport === undefined
        ? undefined
        : {
    ...
    function deferredCommit() {
        return banditReport !== undefined && opts.deferExposures === true
            ? {
                ...
                commitExposures: (shownDzIds) => {
                    const narrowed = narrowBanditReport(banditReport, shownDzIds);
                    const ts2 = new Date().toISOString();
                    try {
                        recordExposures(projectRoot, shownDzIds.map((dzId) => ({ dzId, contextKey: narrowed.contextKey, ts: ts2 })), narrowed.exploredDzIds, { moved: narrowed.moved, arms: narrowed.armsConsidered });
                    }
                    catch { /* a derived index never blocks the recall return (NFR-5) */ }
                },
            }
...
export async function teachGuard(projectRoot, text, opts = {}) {
    ...
    const timeoutMs = opts.timeoutMs ?? DEFAULT_VECTOR_TIMEOUT_MS;
    const result = await withVectorTimeout(searchAgentdbPatterns(projectRoot, text, { limit: 1 }), timeoutMs, () => ({ hits: [], error: `teach guard timed out after ${timeoutMs}ms` }));
    if (result.error !== undefined)
        return { action: 'teach', reason: result.error };
    const hit = result.hits[0];
    if (hit === undefined || hit.dzId === undefined || hit.similarity < threshold)
        return { action: 'teach' };
...
function semanticClusters(items, vecs, threshold) {
    ...
    for (const comp of connectedComponents(n, edges)) {
        ...
        if (c.drops.length > 0)
            clusters.push(c); // all members carved out as reward-forks ⇒ no-op cluster
    }
    return clusters;
}
...
function exactClusters(items) {
    ...
    for (const indices of byText.values()) {
        ...
        if (c.drops.length > 0)
            clusters.push(c); // same-text reward-forks are preserved, not merged
    }
    return clusters;
}
...
function guardRvfEmbedSpace(projectRoot, idmap) {
    const configured = resolveEmbedModel(projectRoot);
    if ('error' in configured)
...
function writeRvfSidecars(projectRoot, idmap, configured, version) {
