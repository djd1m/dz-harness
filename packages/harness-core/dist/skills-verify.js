...
export function verifyRegistration(evidence, options = {}) {
    ...
    if (expectedCommands.length > 0 && facts.slashCommands === null) {
        return fail('inconclusive', 'the init listing has no readable `slash_commands` array (absent or not all strings) — command registration cannot be observed', { ...withFacts, controls: { cwdMatched: true, skillsListPresent: true, commandsListPresent: false } });
    }
    const controls = { cwdMatched: true, skillsListPresent: true, commandsListPresent: facts.slashCommands !== null };
    ...
    if (missing.length > 0 || missingCommands.length > 0) {
        const parts = [];
        if (missing.length > 0)
            parts.push(`${missing.length} of ${expected.length} expected skill(s) did NOT register`);
    ...
    return {
        ...
        missing: [],
        verdict: 'pass',
        reason: expected.length === 0 && expectedCommands.length === 0
...
export function renderRegistrationReport(result, scan) {
    ...
    if (result.expectedCommands.length) {
        out.push(`  commands: ${result.expectedCommands.length} expected` +
            (result.controls.commandsListPresent ? '' : ' · slash_commands listing UNREADABLE'));
        for (const name of result.missingCommands)
 ...
 */
export function declaredPluginSurface(pluginDir) {
    let obj;
    try {
        obj = JSON.parse(readFileSync(join(pluginDir, '.claude-plugin', 'plugin.json'), 'utf8'));
    }
    catch {
        return null;
    }
...
export function buildContentProbePrompt(expected, sample = 6) {
 ...
 */
export function classifyContentProbe(answer, expected) {
    ...
    return {
        ...
        missing,
        quote,
        note: seen.length === expected.length
...
export function renderContentProbe(r) {
    const out = [`  content probe (ADVISORY, model-mediated — never a gate): ${r.verdict} — ${r.note}`];
    if (r.quote)
        out.push(`    quoted: "${r.quote.slice(0, 100)}"`);
    if (r.missing.length)
        out.push(`    not named: ${r.missing.join(', ')}`);
    return out.join('\n');
}
