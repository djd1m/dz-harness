...
| `reqe` | `shouldEmitReqeDebt`, `buildReqeDebt`, `parseReqeDebt`, `buildReqeBrief`, `extractReportGrade`, `settleReqeDebt`, `classifyReqeFindings`, `parsePriorFindings`, `buildReqeVerdict`, `renderReqeList`, `REQE_SCOPE` | The pure half of `dz reqe` — the re-QE debt ledger: when feature-adr's usage-adaptive override made Step-8 QE run on the coder's OWN family (the cross-model guard consciously suspended, FR-2.9), the run records a debt in `features/<slug>/.fa-state/reqe-due.json`. Emission is the NARROW case only (same-family + the ` (usage-switched)` label — never every switch, never the no-override Claude belt); settlement is FAIL-CLOSED: an existing, non-trivial report naming exactly ONE line-anchored grade (`GRADE A-F` boilerplate and `A through F` ranges refused, ambiguity refused), never the run's own 08_qe_report.md. Settlement returns `verdict: ReqeVerdict \| null` (null on refusal): a two-part verdict separates new findings (`ready`/`blocked`/`unassessed`) from prior findings (`closed`/`open`/`unassessed`), using the reviewer's declarations. Debts carry the emitting run's stamp so an old settlement never immunizes a fresh run. HONEST SCOPE printed everywhere: nothing re-runs QE automatically; the validator proves procedural soundness, not authorship. RU: снятый под лимитом гард «кодер не ревьюит сам себя» становится долгом на диске, а не памяткой |
...
**Locks never seed a store (feature `lock-never-seeds-store`, harness-core 0.8.41).** `storeExists(root)` — `existsSync(<root>/.dz)` — is the ONE definition of "a store is here", shared by `store-lock.ts` and `named-lock.ts`. Two lock classes, no default: `withProjectLockSync(projectRoot, name, fn)` and `withStoreLock*` are STORE-SCOPED — they take `<root>/.dz/locks/<name>.lock`, create `locks/` lazily inside an existing store, and throw `StoreAbsentError` (`code: 'ESTOREABSENT'`, with `root` and `lock`) when `<root>/.dz` does not exist, so a lock taken from a wrong cwd can no longer create a store there; `withDirLockSync(dir, name, fn)` is a plain DIRECTORY MUTEX at `<dir>/.dz-locks/<name>.lock` (a pre-existing legacy `<dir>/.dz/locks/` is used while it exists, never created) — it never asks about a store and never creates a directory named `.dz`, which closes the nested `.dz/…/.dz` debris that used to turn a store's subdirectories into `findProjectRoot` traps. The store is created only by its writers (`dz init`, the first `dz teach`, `dz backlog add`). MEASURED before the change (2026-09-21): `packages/@dzhechkov/harness-cli/.dz/locks` was born from a test run without `--project`, and five nested `.dz` dirs sat inside this repo's own store.
...
transaction runs inside `withDirLockSync(dirname(verdictsPath), 'mutation-gate-verdicts', …)` (a directory mutex at `<dirname(verdictsPath)>/.dz-locks/mutation-gate-verdicts.lock`; the
...
The Codex hook root walk selects the nearest `.dz` directory up to and including the repository boundary, stops there or at `$HOME`, and never returns `$HOME`; `.git` only stops the walk and never activates a hook (AM-25).
A shared `.dz` above a checkout (for example, one store over several checkouts) no longer activates its hooks: put a `.dz` in the checkout to opt in.
...
under `withDirLockSync(dirname(dbFile), 'store-generation', …)` (`named-lock.ts` — the repo's
...
- `withAgentdbSnapshotLock(dbFile, fn, opts?)` — a thin, `dbFile`-addressed wrapper over
  `withDirLockSync`: the lock lives at `<dirname(dbFile)>/.dz-locks/agentdb-snapshot.lock` (a legacy `<dirname(dbFile)>/.dz/locks/` is used while it exists, never created), a pure
...
- **Serialized.** The whole read → compare → append → write transaction runs inside
  `withProjectLockSync(root, 'fa-phase-slot', …)` (store-scoped — refuses with `StoreAbsentError` when `<root>/.dz` is absent); pattern counting and directory housekeeping stay
...
**Canonical defects in skill drift and sync.** Both `SweepResult` and `SyncResult` expose
`canonicalDefects: readonly CanonicalDefect[]` (empty for a clean canonical). The shared pure
detector `findCanonicalDefects(canonicalDir, skill)` reports target enrichment assets outside
their owning install root: for example, `agents/openai.yaml` in `skills-meta/foo` is a defect,
while the same asset in `.agents/skills/foo` is owned and remains exempt from propagation.
`CanonicalDefect` carries `skill`, `kind: 'misplaced-enrichment-asset'` (`CanonicalDefectKind`),
skill-relative POSIX `path`, and absolute `canonical`.

The sweep detects against `packages/@dzhechkov/skills-meta/<skill>` when present in the group;
it never auto-picks a canonical. Sync detects against its resolved `from`, `skills-meta`, or
`auto` canonical; resolution `none` returns `canonicalDefects: []`. Per ADR-001 D2(b), a defect
path is excluded from peer comparison only in the canonical `skills-meta` copy: when it is the
only difference, `drifted` is `[]` for sweep / `0` for sync, and `canonicalDefects` reports it.
The same misplaced path in another package copy still counts as drift. The `no-skill-drift`
guard reads `canonicalDefects` separately, so the defect still blocks the gate.

Both CLI commands include `canonicalDefects` in `--json` and print the same line per defect
(example canonical relative to the project root):
...
A canonical defect makes `dz drift-check` and `dz sync-canonical` exit **1** in text and JSON
modes, including `--check`. Sync write mode refuses the entire write, returning `synced: 0`
and `wrote: []` with all copies unchanged; it still reports drift as check mode does. Canonical
defects are never healed, propagated, or removed by these commands. A repo with an existing
misplaced asset can therefore fail the gate immediately; inspect `dz drift-check --json`.
...
(`withProjectLockSync` for data under a project's `.dz/`, `withDirLockSync` for read-modify-write file stores outside the worktree). Full detail in
