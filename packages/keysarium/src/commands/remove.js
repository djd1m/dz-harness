...
/**
 * `@dzhechkov/keysarium remove` — Clean uninstall of the toolkit.
 *
...
async function run(options) {
  ...
  if (!fileExists(manifestPath)) {
    warn('Keysarium is not installed in this directory — nothing to remove.');
    process.exit(0);
  }
  ...
  if (!manifest) {
    ...
    info('You can manually delete .keysarium.json and the installed files.');
    process.exit(1);
  }
  ...
  console.log('');
  info(bold('The following will be removed:'));
  console.log('');
  ...
  console.log(`  ${dim('+ .keysarium.json manifest')}`);
  console.log('');
  ...
  if (!force) {
    const confirmed = await confirmPrompt(
      yellow('This will remove all Keysarium files. Continue? (y/N) ')
    );
  ...
  info('Removed .keysarium.json manifest');

  // ── f) Summary ─────────────────────────────────────────────────────────
  console.log('');
  success(bold('Removal complete!'));
  console.log(`  ${green('\u2713')} ${removedCount} file(s) removed`);
  if (skippedCount > 0) {
    console.log(`  ${dim('-')} ${skippedCount} file(s) already missing (skipped)`);
  }
  console.log('');
  info(`To reinstall, run: ${cyan('npx @dzhechkov/keysarium init')}`);
  console.log('');
