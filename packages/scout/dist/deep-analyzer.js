...
function findClosestMatch(description, ourSkillDescriptions) {
...
function recommendIntegration(skillId, closestMatch, stars) {
...
async function fetchGitHubFile(fullName, path, branch, token) {
    const url = `${GITHUB_RAW}/${fullName}/${branch}/${path}`;
    const headers = { 'User-Agent': 'dz-scout/0.2.0' };
    if (token)
...
async function fetchSkillPaths(fullName, branch, token) {
...
const OUR_DESCRIPTIONS = new Map([
 ...
 */
export async function deepAnalyze(repos, options = {}) {
    const minScore = options.minScore ?? 50;
    // Only GitHub repos can be deep-analyzed (need owner/repo format for tree API)
    const candidates = repos.filter((r) => r.relevanceScore >= minScore &&
        r.fullName.includes('/') &&
        ...
        for (const skillPath of skillPaths.slice(0, 5)) { // max 5 skills per repo
            ...
            for (const topic of repo.topics) {
                ...
                if (entry.examples.length < 3)
                    entry.examples.push(repo.fullName);
    ...
    const gaps = [...categoryCount.entries()]
...
function generateDeepMarkdown(analyses, gaps) {
    const lines = [
        '',
        '---',
        '',
