...
export async function scanEcc(options = {}) {
    ...
    try {
        ...
        const repo = await fetchJsonWithBudget(ECC_API, { headers });
        ...
        if (repo.topics === undefined)
            topics = [];
        else if (isStringList(repo.topics))
            topics = repo.topics;
        else
            throw new SourceRefusal('refused', `${ECC_API}: поле topics не список строк — измерения нет`);
        ...
        if (lic === undefined || lic === null)
            license = null;
        else if (typeof lic === 'object' && typeof lic.spdx_id === 'string') {
            license = lic.spdx_id;
        }
        else
            throw new SourceRefusal('refused', `${ECC_API}: поле license не той формы — измерения нет`);
    }
    catch (err) {
    ...
    const skillNames = [];
    try {
        const entries = await fetchJsonWithBudget(ECC_SKILLS_API, { headers });
