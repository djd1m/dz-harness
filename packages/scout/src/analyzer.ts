...
const FORMAT_WEIGHTS: Record<SkillFormat, number> = {
  'agentskills-io': 1.0,
  'claude-plugin': 0.9,
  'claude-skills': 0.85,
  'codex-skills': 0.7,
...
export function detectFormats(item: GitHubSearchItem): SkillFormat[] {
...
export function computeRelevanceScore(
  ...
  const recencyScore =
    (daysSinceCommit < 30 ? 1.0 : daysSinceCommit < 90 ? 0.7 : daysSinceCommit < 365 ? 0.3 : 0.1) * 20;
...
export function recommend(score: number, stars: number): Recommendation {
  if (score >= 70) return 'integrate';
  if (score >= 40 && stars >= 50) return 'monitor';
  return 'skip';
}

/** Extract potential skill IDs from topics (heuristic). */
export function extractSkillHints(item: GitHubSearchItem): string[] {
  return item.topics
...
export function analyzeRepo(item: GitHubSearchItem): RepoProfile {
