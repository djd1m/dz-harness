...
export class SourceRefusal extends Error {
  ...
  constructor(kind: RefusalKind, message: string, extra?: { status?: number; budgetMs?: number }) {
    super(message);
    this.name = 'SourceRefusal';
    this.kind = kind;
    if (extra?.status !== undefined) this.status = extra.status;
    if (extra?.budgetMs !== undefined) this.budgetMs = extra.budgetMs;
  }
}
 ...
 */
export function isSourceRefusal(err: unknown): err is SourceRefusal {
  const kind = (err as { kind?: unknown } | null)?.kind;
  return typeof err === 'object' && err !== null
    && (err as { name?: unknown }).name === 'SourceRefusal'
    && typeof kind === 'string' && (REFUSAL_KINDS as readonly string[]).includes(kind);
}

/** Перечень форм отказа. Закрыт: новая форма — осознанная правка здесь. */
export const REFUSAL_KINDS = ['refused', 'timeout', 'failed'] as const;
...
export async function fetchWithBudget(
  url: string,
  init: RequestInit = {},
  budgetMs: number = SOURCE_BUDGET_MS,
): Promise<Response> {
...
function combineSignals(ours: AbortSignal, theirs: AbortSignal | undefined): AbortSignal {
 ...
 */
async function readWithBudget(url: string, init: RequestInit, budgetMs: number, as: 'text' | 'json'): Promise<unknown> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), budgetMs);
  const signal = combineSignals(controller.signal, init.signal ?? undefined);
  try {
...
export async function fetchTextWithBudget(
  url: string, init: RequestInit = {}, budgetMs: number = SOURCE_BUDGET_MS,
): Promise<string> {
  return await readWithBudget(url, init, budgetMs, 'text') as string;
}
...
export async function fetchJsonWithBudget<T = unknown>(
  ...
  return await readWithBudget(url, init, budgetMs, 'json') as T;
}
 ...
 */
export function refuseIfNothingMeasured(
  measured: number,
  refusals: readonly SourceRefusal[],
  label: string,
): void {
  const first = refusals[0];
  if (measured === 0 && first !== undefined) throw first;
  if (first !== undefined) {
    console.error(
      `${label}: измерено ${measured}, но ${refusals.length} из обращений отказали `
