...
export function updateFromHeaders(state: RateLimitState, headers: Headers): void {
...
export function getDelay(state: RateLimitState): number {
...
export function shouldRetry(state: RateLimitState): boolean {
  return state.retryCount < MAX_RETRIES;
}

/** Mark a retry attempt. */
export function markRetry(state: RateLimitState): void {
  state.retryCount += 1;
}

/** Reset retry count after a successful request. */
export function markSuccess(state: RateLimitState): void {
  state.retryCount = 0;
}
